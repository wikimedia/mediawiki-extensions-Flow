...
( function ( $, mw ) {
	...
	FlowBoardAndHistoryComponentBase.UI.events.interactiveHandlers.cancelForm = function ( event ) {
		var target = this,
			...
			eventlogFunnelId = $( this ).data( 'flow-eventlog-funnel' );

		flowComponent.logEvent( { flowEventlogAction: 'cancel-attempt', flowEventlogFunnel: eventlogFunnelId } );
		...
		if ( changedFieldCount && !confirm( flowComponent.constructor.static.TemplateEngine.l10n( 'flow-cancel-warning' ) ) ) {
			flowComponent.logEvent( { flowEventlogAction: 'cancel-abort', flowEventlogFunnel: eventlogFunnelId } );
		...
		if ( changedFieldCount ) {
			flowComponent.logEvent( { flowEventlogAction: 'cancel-success', flowEventlogFunnel: eventlogFunnelId } );
		}
