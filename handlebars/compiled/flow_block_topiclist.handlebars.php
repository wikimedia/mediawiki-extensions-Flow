...
' : '').'
	...
	<div class="flow-topic-titlebar flow-click-interactive" data-flow-interactive-handler="topicCollapserToggle" tabindex="0">
		...
		<div class="flow-menu">
			...
			<ul class="flow-ui-button-container">
				...
				' : '').'
				'.((LCRun3::ifvar($cx, ((is_array($in['actions']) && isset($in['actions']['restore'])) ? $in['actions']['restore'] : null))) ? '
					<li>
						<a class="flow-ui-button flow-ui-progressive flow-ui-quiet flow-ui-thin"
						   href="'.htmlentities(((is_array($in['actions']['restore']) && isset($in['actions']['restore']['url'])) ? $in['actions']['restore']['url'] : null), ENT_QUOTES, 'UTF-8').'"
						   title="'.htmlentities(((is_array($in['actions']['restore']) && isset($in['actions']['restore']['title'])) ? $in['actions']['restore']['title'] : null), ENT_QUOTES, 'UTF-8').'"
						   data-flow-interactive-handler="moderationDialog"
						   data-template="flow_moderate_topic"
						   data-role="restore">
							<span class="wikiglyph wikiglyph-unknown"></span>
							'.LCRun3::ch($cx, 'l10n', Array('flow-post-action-restore-post'), 'encq').'
						</a>
					</li>
				' : '').'
	...
	<span class="flow-ui-tooltip-triangle"></span>
		...
		<textarea id="flow-post-'.htmlentities(((is_array($in) && isset($in['postId'])) ? $in['postId'] : null), ENT_QUOTES, 'UTF-8').'-form-content"
				...
				class="mw-ui-input"
				type="text"
				placeholder="'.LCRun3::ch($cx, 'l10n', Array('Reply_to_author_name',((is_array($in) && isset($in['author'])) ? $in['author'] : null)), 'encq').'"
				data-role="content">'.LCRun3::hbch($cx, 'ifEquals', Array(((is_array($cx['scopes'][0]['submitted']) && isset($cx['scopes'][0]['submitted']['postId'])) ? $cx['scopes'][0]['submitted']['postId'] : null),((is_array($in) && isset($in['postId'])) ? $in['postId'] : null)), $in, function($cx, $in) {return '
