<?php return function ($in) {
	...
	<form class="flow-edit-form" data-flow-initial-state="collapsed" method="POST" action="'.htmlentities(((is_array($in['revision']['actions']['edit']) && isset($in['revision']['actions']['edit']['url'])) ? $in['revision']['actions']['edit']['url'] : null), ENT_QUOTES, 'UTF-8').'">
		...
		<div class="flow-form-actions flow-form-collapsible">
			<button data-role="submit" class="flow-ui-button flow-ui-constructive">'.LCRun2::ch('l10n', Array('flow-topic-action-summarize-topic'), 'enc', $cx).'</button>
			...
			<small class="flow-terms-of-use plainlinks">'.LCRun2::ch('l10n', Array('flow-terms-of-use-summarize'), 'enc', $cx).'</small>
		</div>
	</form>
</div>
';
