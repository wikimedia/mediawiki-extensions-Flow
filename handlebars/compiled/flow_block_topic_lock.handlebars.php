...
'.$sp.'				<input type="hidden" name="flow_reason" value="'.((LR::ifvar($cx, ((isset($in['isLocked']) && is_array($in)) ? $in['isLocked'] : null), false)) ? ''.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-rev-message-restore-topic-reason'),array()), 'encq', $in)).'' : ''.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-rev-message-lock-topic-reason'),array()), 'encq', $in)).'').'" />
'.$sp.'				<div class="flow-form-actions flow-form-collapsible">
'.$sp.'					<button data-role="submit"
'.$sp.'					        class="mw-ui-button mw-ui-constructive"
'.$sp.'					>
'.$sp.''.((LR::ifvar($cx, ((isset($in['isLocked']) && is_array($in)) ? $in['isLocked'] : null), false)) ? '							'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-topic-action-unlock-topic'),array()), 'encq', $in)).'
...
'.$sp.''.((LR::ifvar($cx, ((isset($in['isLocked']) && is_array($in)) ? $in['isLocked'] : null), false)) ? '							'.LR::encq($cx, LR::hbch($cx, 'l10nParse', array(array('flow-terms-of-use-unlock-topic'),array()), 'encq', $in)).'
