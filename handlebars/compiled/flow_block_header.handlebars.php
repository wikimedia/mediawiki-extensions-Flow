...
'.$sp.'</h2>
';},'flow_header_edit_restrictions' => function ($cx, $in, $sp) {return ''.$sp.''.((!LCRun3::ifvar($cx, ((isset($in['revision']['actions']['edit']) && is_array($in['revision']['actions'])) ? $in['revision']['actions']['edit'] : null))) ? '	<p class="flow-board-description-restricted">
'.$sp.'		'.LCRun3::ch($cx, 'oouify', array(array('lock'),array('type'=>'IconWidget','classes'=>'flow-board-description-restricted-icon')), 'raw').'
'.$sp.'
'.$sp.'		<span class="flow-board-description-restricted-label">'.LCRun3::ch($cx, 'l10n', array(array('flow-board-header-can-not-edit'),array()), 'encq').'</span>
'.$sp.'        </p>
