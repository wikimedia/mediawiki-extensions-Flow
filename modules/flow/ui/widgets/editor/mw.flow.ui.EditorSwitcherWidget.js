( function ( $ ) {
	...
	mw.flow.ui.EditorSwitcherWidget.prototype.getNextEditor = function ( editorKey ) {
		...
		existingEditor = this.getActiveEditor().key;
	...
	mw.flow.ui.EditorSwitcherWidget.prototype.setEditor = function ( editor, content ) {
		...
		// Add 'in progress' to the active editor
		currentEditor.widget.addClass( 'oo-ui-texture-pending' );
		...
		return this.setActiveEditor( editor )
			...
			.always( function () {
				// Remove the 'in progress' indication from the current editor
				currentEditor.widget.removeClass( 'oo-ui-texture-pending' );
			} );
	};
