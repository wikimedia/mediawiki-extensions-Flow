...
use \LightnCandy\SafeString as SafeString;use \LightnCandy\Runtime as LR;return function ($in = null, $options = null) {
    ...
    $partials = array('flow_board_navigation' => function ($cx, $in, $sp) {return ''.$sp.''.((LR::ifvar($cx, ((isset($in['links']) && is_array($in['links']) && isset($in['links']['board-sort'])) ? $in['links']['board-sort'] : null), false)) ? '<div class="flow-board-navigation" data-flow-load-handler="boardNavigation">
...
'.$sp.''.LR::hbbch($cx, 'tooltip', array(array(),array('positionClass'=>'down','contextClass'=>'progressive','extraClass'=>'flow-form-collapsible','isBlock'=>true)), $in, false, function($cx, $in)use($sp){return ''.LR::encq($cx, LR::hbch($cx, 'l10nParse', array(array('flow-anon-warning',LR::hbch($cx, 'linkWithReturnTo', array(array('Special:UserLogin'),array()), 'raw', $in),LR::hbch($cx, 'linkWithReturnTo', array(array('Special:UserLogin/signup'),array()), 'raw', $in)),array()), 'encq', $in)).'';}).'	</div>
'.$sp.'
'.$sp.''.LR::hbbch($cx, 'progressiveEnhancement', array(array(),array()), $in, false, function($cx, $in)use($sp){return '		<div class="flow-anon-warning-desktop">
'.$sp.''.LR::hbbch($cx, 'tooltip', array(array(),array('positionClass'=>'left','contextClass'=>'progressive','extraClass'=>'flow-form-collapsible','isBlock'=>true)), $in, false, function($cx, $in)use($sp){return ''.LR::encq($cx, LR::hbch($cx, 'l10nParse', array(array('flow-anon-warning',LR::hbch($cx, 'linkWithReturnTo', array(array('Special:UserLogin'),array()), 'raw', $in),LR::hbch($cx, 'linkWithReturnTo', array(array('Special:UserLogin/signup'),array()), 'raw', $in)),array()), 'encq', $in)).'';}).'		</div>
'.$sp.'';}).'</div>
';},
...
'.$sp.'			data-role="title"
'.$sp.'		/>
'.$sp.''.((!LR::ifvar($cx, ((is_array($in) && isset($in['isOnFlowBoard'])) ? $in['isOnFlowBoard'] : null), false)) ? '			<div class="flow-editor">
...
'.$sp.''.LR::p($cx, 'flow_form_cancel_button', array(array($in),array()),0, '				').'				<small class="flow-terms-of-use plainlinks">'.LR::encq($cx, LR::hbch($cx, 'l10nParse', array(array('flow-terms-of-use-new-topic'),array()), 'encq', $in)).'</small>
'.$sp.'			</div>
'.$sp.'' : '').'	</form>
'.$sp.'' : '').'';},
'flow_topic_moderation_flag' => function ($cx, $in, $sp) {return ''.$sp.'<span class="mw-ui-icon mw-ui-icon-before'.LR::hbbch($cx, 'ifCond', array(array(((is_array($in) && isset($in['moderateState'])) ? $in['moderateState'] : null),'===','lock'),array()), $in, false, function($cx, $in)use($sp){return ' mw-ui-icon-check';}).''.LR::hbbch($cx, 'ifCond', array(array(((is_array($in) && isset($in['moderateState'])) ? $in['moderateState'] : null),'===','hide'),array()), $in, false, function($cx, $in)use($sp){return ' mw-ui-icon-flag';}).''.LR::hbbch($cx, 'ifCond', array(array(((is_array($in) && isset($in['moderateState'])) ? $in['moderateState'] : null),'===','delete'),array()), $in, false, function($cx, $in)use($sp){return ' mw-ui-icon-trash';}).'"></span>
';},
'flow_post_moderation_state' => function ($cx, $in, $sp) {return ''.$sp.'<span class="plainlinks">'.((LR::ifvar($cx, ((is_array($in) && isset($in['replyToId'])) ? $in['replyToId'] : null), false)) ? ''.LR::encq($cx, LR::hbch($cx, 'l10nParse', array(array(LR::hbch($cx, 'concat', array(array('flow-',((is_array($in) && isset($in['moderateState'])) ? $in['moderateState'] : null),'-post-content'),array()), 'raw', $in),((isset($in['moderator']) && is_array($in['moderator']) && isset($in['moderator']['name'])) ? $in['moderator']['name'] : null),((isset($in['links']['topic-history']) && is_array($in['links']['topic-history']) && isset($in['links']['topic-history']['url'])) ? $in['links']['topic-history']['url'] : null)),array()), 'encq', $in)).'' : ''.LR::encq($cx, LR::hbch($cx, 'l10nParse', array(array(LR::hbch($cx, 'concat', array(array('flow-',((is_array($in) && isset($in['moderateState'])) ? $in['moderateState'] : null),'-title-content'),array()), 'raw', $in),((isset($in['moderator']) && is_array($in['moderator']) && isset($in['moderator']['name'])) ? $in['moderator']['name'] : null),((isset($in['links']['topic-history']) && is_array($in['links']['topic-history']) && isset($in['links']['topic-history']['url'])) ? $in['links']['topic-history']['url'] : null)),array()), 'encq', $in)).'').'</span>
';},
'flow_topic_titlebar_content' => function ($cx, $in, $sp) {return ''.$sp.'<div class="flow-topic-titlebar-container">
'.$sp.'    <h2 class="flow-topic-title flow-load-interactive '.((LR::ifvar($cx, ((is_array($in) && isset($in['isLocked'])) ? $in['isLocked'] : null), false)) ? 'flow-collapse-toggle flow-click-interactive' : '').'"
...
'.$sp.''.((LR::ifvar($cx, ((is_array($in) && isset($in['isLocked'])) ? $in['isLocked'] : null), false)) ? '        data-flow-interactive-handler="collapserCollapsibleToggle"
'.$sp.'' : '').'            >
'.$sp.'		'.((LR::ifvar($cx, ((is_array($in) && isset($in['isLocked'])) ? $in['isLocked'] : null), false)) ? '<span class="mw-ui-icon mw-ui-icon-before mw-ui-icon-check"></span> ' : '').''.LR::encq($cx, LR::hbch($cx, 'escapeContent', array(array(((isset($in['content']) && is_array($in['content']) && isset($in['content']['format'])) ? $in['content']['format'] : null),((isset($in['content']) && is_array($in['content']) && isset($in['content']['content'])) ? $in['content']['content'] : null)),array()), 'encq', $in)).'</h2>
'.$sp.'    <div class="flow-topic-meta">
'.$sp.''.((LR::ifvar($cx, ((is_array($in) && isset($in['isLocked'])) ? $in['isLocked'] : null), false)) ? '<a class="expand-collapse-posts-link flow-collapse-toggle flow-click-interactive"
'.$sp.'               href="javascript:void(0);"
'.$sp.'               title="'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-show-comments-title',((is_array($in) && isset($in['reply_count'])) ? $in['reply_count'] : null)),array()), 'encq', $in)).'"
'.$sp.'               data-collapsed-title="'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-show-comments-title',((is_array($in) && isset($in['reply_count'])) ? $in['reply_count'] : null)),array()), 'encq', $in)).'"
'.$sp.'               data-expanded-title="'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-hide-comments-title',((is_array($in) && isset($in['reply_count'])) ? $in['reply_count'] : null)),array()), 'encq', $in)).'"
'.$sp.'               data-flow-interactive-handler="collapserCollapsibleToggle"
'.$sp.'                    >'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-topic-comments',((is_array($in) && isset($in['reply_count'])) ? $in['reply_count'] : null)),array()), 'encq', $in)).'</a>' : ''.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-topic-comments',((is_array($in) && isset($in['reply_count'])) ? $in['reply_count'] : null)),array()), 'encq', $in)).'').' &bull;
'.$sp.'
...
'.$sp.''.((LR::ifvar($cx, ((is_array($in) && isset($in['last_updated'])) ? $in['last_updated'] : null), false)) ? '				'.LR::encq($cx, LR::hbch($cx, 'timestamp', array(array(((is_array($in) && isset($in['last_updated'])) ? $in['last_updated'] : null)),array()), 'encq', $in)).'
'.$sp.'' : '				'.LR::encq($cx, LR::hbch($cx, 'uuidTimestamp', array(array(((is_array($in) && isset($in['postId'])) ? $in['postId'] : null)),array()), 'encq', $in)).'
'.$sp.'').'        </a>
'.$sp.'    </div>
'.$sp.''.((LR::ifvar($cx, ((is_array($in) && isset($in['isModeratedNotLocked'])) ? $in['isModeratedNotLocked'] : null), false)) ? '        <div class="flow-moderated-topic-title flow-ui-text-truncated">'.LR::encq($cx, ((is_array($in) && isset($in['noop'])) ? $in['noop'] : null)).''.LR::p($cx, 'flow_topic_moderation_flag', array(array($in),array()),0).'
...
'.$sp.'        </div>
'.$sp.'' : '').'    <span class="flow-reply-count"><span class="flow-reply-count-number">'.LR::encq($cx, ((is_array($in) && isset($in['reply_count'])) ? $in['reply_count'] : null)).'</span></span>
'.$sp.'</div>';},
'flow_topic_titlebar_summary' => function ($cx, $in, $sp) {return ''.$sp.'<div class="flow-topic-summary-container '.((LR::ifvar($cx, ((is_array($in) && isset($in['isLocked'])) ? $in['isLocked'] : null), false)) ? 'flow-collapse-toggle flow-click-interactive' : '').'"
'.$sp.''.((LR::ifvar($cx, ((is_array($in) && isset($in['isLocked'])) ? $in['isLocked'] : null), false)) ? '		data-flow-interactive-handler="collapserCollapsibleToggle"
'.$sp.'' : '').'		>
...
'.$sp.''.LR::hbbch($cx, 'ifCond', array(array(((isset($in['revision']['creator']) && is_array($in['revision']['creator']) && isset($in['revision']['creator']['name'])) ? $in['revision']['creator']['name'] : null),'===',((isset($in['revision']['author']) && is_array($in['revision']['author']) && isset($in['revision']['author']['name'])) ? $in['revision']['author']['name'] : null)),array()), $in, false, function($cx, $in)use($sp){return '					'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-summary-authored',((isset($in['revision']['creator']) && is_array($in['revision']['creator']) && isset($in['revision']['creator']['name'])) ? $in['revision']['creator']['name'] : null)),array()), 'encq', $in)).'
'.$sp.'';}, function($cx, $in)use($sp){return '					'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-summary-edited',((isset($in['revision']['author']) && is_array($in['revision']['author']) && isset($in['revision']['author']['name'])) ? $in['revision']['author']['name'] : null)),array()), 'encq', $in)).'
...
'.$sp.''.LR::p($cx, 'flow_errors', array(array($in),array()),0, '	').'
'.$sp.'	<a href="'.((LR::ifvar($cx, ((is_array($in) && isset($in['isWatched'])) ? $in['isWatched'] : null), false)) ? ''.LR::encq($cx, ((isset($in['links']['unwatch-topic']) && is_array($in['links']['unwatch-topic']) && isset($in['links']['unwatch-topic']['url'])) ? $in['links']['unwatch-topic']['url'] : null)).'' : ''.LR::encq($cx, ((isset($in['links']['watch-topic']) && is_array($in['links']['watch-topic']) && isset($in['links']['watch-topic']['url'])) ? $in['links']['watch-topic']['url'] : null)).'').'"
'.$sp.'	   class="mw-ui-anchor mw-ui-hovericon '.((!LR::ifvar($cx, ((is_array($in) && isset($in['isWatched'])) ? $in['isWatched'] : null), false)) ? 'mw-ui-quiet' : '').'
'.$sp.''.((LR::ifvar($cx, ((is_array($in) && isset($in['isWatched'])) ? $in['isWatched'] : null), false)) ? 'flow-watch-link-unwatch' : 'flow-watch-link-watch').'"
'.$sp.'	   data-flow-api-handler="watchItem"
'.$sp.'	   data-flow-api-target="< .flow-topic-watchlist"
'.$sp.'	   data-flow-api-method="POST">'.LR::encq($cx, ((is_array($in) && isset($in['noop'])) ? $in['noop'] : null)).'<span class="flow-unwatch mw-ui-icon mw-ui-icon-before mw-ui-icon-unStar mw-ui-icon-unStar-progressive" title="'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-topic-action-watchlist-remove'),array()), 'encq', $in)).'"></span>'.LR::encq($cx, ((is_array($in) && isset($in['noop'])) ? $in['noop'] : null)).''.LR::encq($cx, ((is_array($in) && isset($in['noop'])) ? $in['noop'] : null)).'<span class="flow-watch mw-ui-icon mw-ui-icon-before mw-ui-icon-star mw-ui-icon-star-progressive-hover" title="'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-topic-action-watchlist-add'),array()), 'encq', $in)).'"></span>'.LR::encq($cx, ((is_array($in) && isset($in['noop'])) ? $in['noop'] : null)).'</a>
'.$sp.'</div>
';},
...
'.$sp.''.LR::p($cx, 'flow_topic_titlebar_content', array(array($in),array()),0, '	').''.LR::p($cx, 'flow_topic_titlebar_summary', array(array(((is_array($in) && isset($in['summary'])) ? $in['summary'] : null)),array('isLocked'=>((is_array($in) && isset($in['isLocked'])) ? $in['isLocked'] : null))),0, '	').''.((LR::ifvar($cx, ((is_array($in) && isset($in['watchable'])) ? $in['watchable'] : null), false)) ? ''.LR::p($cx, 'flow_topic_titlebar_watch', array(array($in),array()),0, '		').'' : '').'	<div class="flow-menu flow-menu-hoverable">
...
'.$sp.'</div>
';},
'flow_reply_form' => function ($cx, $in, $sp) {return ''.$sp.''.((!LR::ifvar($cx, ((isset($in['actions']) && is_array($in['actions']) && isset($in['actions']['unlock'])) ? $in['actions']['unlock'] : null), false)) ? '
...
'.$sp.'      id="flow-reply-'.LR::encq($cx, ((is_array($in) && isset($in['postId'])) ? $in['postId'] : null)).'"
...
'.$sp.'	<input type="hidden" name="topic_replyTo" value="'.LR::encq($cx, ((is_array($in) && isset($in['postId'])) ? $in['postId'] : null)).'" />
'.$sp.''.LR::p($cx, 'flow_errors', array(array($in),array()),0, '	').'
'.$sp.''.LR::hbbch($cx, 'ifAnonymous', array(array(),array()), $in, false, function($cx, $in)use($sp){return ''.LR::p($cx, 'flow_anon_warning', array(array($in),array()),0, '		').'';}).'
'.$sp.'	<div class="flow-editor">
'.$sp.'		<textarea id="flow-post-'.LR::encq($cx, ((is_array($in) && isset($in['postId'])) ? $in['postId'] : null)).'-form-content"
...
'.$sp.'		          data-role="content"
'.$sp.'
'.$sp.'		>'.((LR::ifvar($cx, (isset($cx['sp_vars']['root']['submitted']) ? $cx['sp_vars']['root']['submitted'] : null), false)) ? ''.LR::hbbch($cx, 'ifCond', array(array((isset($cx['sp_vars']['root']['submitted']['postId']) ? $cx['sp_vars']['root']['submitted']['postId'] : null),'===',((is_array($in) && isset($in['postId'])) ? $in['postId'] : null)),array()), $in, false, function($cx, $in)use($sp){return ''.LR::encq($cx, (isset($cx['sp_vars']['root']['submitted']['content']) ? $cx['sp_vars']['root']['submitted']['content'] : null)).'';}).'' : '').'</textarea>
'.$sp.'	</div>
'.$sp.'
'.$sp.'	<div class="flow-form-actions flow-form-collapsible">
'.$sp.'		<button data-role="submit"
...
'.$sp.''.LR::p($cx, 'flow_form_cancel_button', array(array($in),array()),0, '		').'		<small class="flow-terms-of-use plainlinks">'.LR::encq($cx, LR::hbch($cx, 'l10nParse', array(array('flow-terms-of-use-reply'),array()), 'encq', $in)).'</small>
'.$sp.'	</div>
'.$sp.'</form>
'.$sp.'' : '').'';},
'flow_topic' => function ($cx, $in, $sp) {return ''.$sp.'<div class="flow-topic flow-load-interactive'.((LR::ifvar($cx, ((is_array($in) && isset($in['moderateState'])) ? $in['moderateState'] : null), false)) ? ' flow-topic-moderatestate-'.LR::encq($cx, ((is_array($in) && isset($in['moderateState'])) ? $in['moderateState'] : null)).' ' : '').''.((LR::ifvar($cx, ((is_array($in) && isset($in['isModerated'])) ? $in['isModerated'] : null), false)) ? ' flow-topic-moderated ' : '').''.((LR::ifvar($cx, ((is_array($in) && isset($in['isLocked'])) ? $in['isLocked'] : null), false)) ? 'flow-element-collapsible flow-element-collapsed' : '').'"
'.$sp.'     id="flow-topic-'.LR::encq($cx, ((is_array($in) && isset($in['postId'])) ? $in['postId'] : null)).'"
'.$sp.'     data-flow-id="'.LR::encq($cx, ((is_array($in) && isset($in['postId'])) ? $in['postId'] : null)).'"
'.$sp.'     data-flow-load-handler="topic"
'.$sp.'     data-flow-toc-scroll-target=".flow-topic-titlebar"
'.$sp.'     data-flow-topic-timestamp-updated="'.LR::encq($cx, ((is_array($in) && isset($in['last_updated'])) ? $in['last_updated'] : null)).'"
'.$sp.'>
'.$sp.''.LR::p($cx, 'flow_topic_titlebar', array(array($in),array()),0, '	').'
'.$sp.''.((LR::ifvar($cx, (isset($cx['sp_vars']['root']['posts']) ? $cx['sp_vars']['root']['posts'] : null), false)) ? ''.LR::sec($cx, ((is_array($in) && isset($in['replies'])) ? $in['replies'] : null), null, $in, true, function($cx, $in)use($sp){return ''.LR::hbbch($cx, 'eachPost', array(array((isset($cx['sp_vars']['root']) ? $cx['sp_vars']['root'] : null),$in),array()), $in, false, function($cx, $in)use($sp){return '				<!-- eachPost topic -->
'.$sp.'				'.LR::encq($cx, LR::hbch($cx, 'post', array(array((isset($cx['sp_vars']['root']) ? $cx['sp_vars']['root'] : null),$in),array()), 'encq', $in)).'
'.$sp.'';}).'';}).'' : '').'
'.$sp.''.((LR::ifvar($cx, ((isset($in['actions']) && is_array($in['actions']) && isset($in['actions']['reply'])) ? $in['actions']['reply'] : null), false)) ? ''.LR::hbbch($cx, 'ifCond', array(array((isset($cx['sp_vars']['root']['submitted']['postId']) ? $cx['sp_vars']['root']['submitted']['postId'] : null),'===',((is_array($in) && isset($in['postId'])) ? $in['postId'] : null)),array()), $in, false, function($cx, $in)use($sp){return ''.LR::p($cx, 'flow_reply_form', array(array($in),array()),0, '			').'';}, function($cx, $in)use($sp){return ''.LR::hbbch($cx, 'progressiveEnhancement', array(array(),array('type'=>'replace','target'=>'~ a')), $in, false, function($cx, $in)use($sp){return ''.LR::p($cx, 'flow_reply_form', array(array($in),array()),0, '				').'';}).'			<a href="'.LR::encq($cx, ((isset($in['actions']['reply']) && is_array($in['actions']['reply']) && isset($in['actions']['reply']['url'])) ? $in['actions']['reply']['url'] : null)).'"
...
'.$sp.'';}).'' : ''.LR::hbbch($cx, 'progressiveEnhancement', array(array(),array('type'=>'insert')), $in, false, function($cx, $in)use($sp){return ''.LR::p($cx, 'flow_reply_form', array(array($in),array()),0, '			').'';}).'').'</div>
';},
'flow_topiclist_loop' => function ($cx, $in, $sp) {return ''.$sp.''.LR::sec($cx, ((is_array($in) && isset($in['roots'])) ? $in['roots'] : null), null, $in, true, function($cx, $in)use($sp){return ''.LR::hbbch($cx, 'eachPost', array(array((isset($cx['sp_vars']['root']) ? $cx['sp_vars']['root'] : null),$in),array()), $in, false, function($cx, $in)use($sp){return ''.LR::p($cx, 'flow_topic', array(array($in),array()),0, '		').'';}).'';}).'';},
'flow_load_more' => function ($cx, $in, $sp) {return ''.$sp.''.((LR::ifvar($cx, ((is_array($in) && isset($in['loadMoreObject'])) ? $in['loadMoreObject'] : null), false)) ? '	<div class="flow-load-more">
'.$sp.'		<div class="flow-error-container">
'.$sp.'		</div>
'.$sp.'
...
'.$sp.'		   data-flow-api-handler="'.LR::encq($cx, ((is_array($in) && isset($in['loadMoreApiHandler'])) ? $in['loadMoreApiHandler'] : null)).'"
'.$sp.'		   data-flow-api-target="< .flow-load-more"
'.$sp.'		   data-flow-load-handler="loadMore"
'.$sp.'		   data-flow-scroll-target="'.LR::encq($cx, ((is_array($in) && isset($in['loadMoreTarget'])) ? $in['loadMoreTarget'] : null)).'"
'.$sp.'		   data-flow-scroll-container="'.LR::encq($cx, ((is_array($in) && isset($in['loadMoreContainer'])) ? $in['loadMoreContainer'] : null)).'"
'.$sp.'		   data-flow-template="'.LR::encq($cx, ((is_array($in) && isset($in['loadMoreTemplate'])) ? $in['loadMoreTemplate'] : null)).'"
...
<div class="flow-board" data-flow-sortby="'.LR::encq($cx, ((is_array($in) && isset($in['sortby'])) ? $in['sortby'] : null)).'">
