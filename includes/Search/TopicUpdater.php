...
class TopicUpdater extends Updater {
	...
	public function buildDocument( /* PostRevision */ $revision ) {
		...
		$updateTimestamp = $revision->getRecursiveResult( $idTimestamp );
		// timestamp for initial topic post
		$creationTimestamp = $revision->getPostId()->getTimestampObj();
	...
	public function getRevisionData( /* PostRevision|PostSummary */ $revision, array $result ) {
		...
		$result[] = array(
			...
			'timestamp' => wfTimestamp( TS_ISO_8601, $revision->getPostId()->getTimestampObj() ),
			'update_timestamp' => wfTimestamp( TS_ISO_8601, $revision->getRevisionId()->getTimestampObj() ),
			'type' => $type,
		);
