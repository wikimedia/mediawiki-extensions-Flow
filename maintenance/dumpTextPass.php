...
require_once $maintPath . '/backupPrefetch.inc';
require_once dirname( __FILE__ ) . '/backupPrefetch.inc';
...
TEXT
		...
		while ( $failures < $this->maxFailures ) {
			...
			try {
				...
				if ( $text === false && isset( $this->prefetch ) && $prefetchNotTried ) {
					...
					$text = $this->prefetch->prefetch( $revId->getHex() );
					if ( $text === null ) {
						$text = false;
					}
				}
	...
	function endElement( $parser, $name ) {
		...
		if ( $name == 'board' ) {
			$this->stack->clear();
		} elseif ( in_array( $name, [ 'topic', 'summary', 'description' ] ) ) {
