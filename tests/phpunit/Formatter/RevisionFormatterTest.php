...
class RevisionFormatterTest extends \MediaWikiTestCase {
	...
	public function testFormattingPreviousRevision() {
		list( $formatter, $ctx ) = $this->mockFormatter();
	...
	public function testFormattingContentLength() {
		...
		// Change permissions to disallow the previous revision
		$permissions = $this->mockPermissions( $actions );
		// formatting only proceedes when this is true
		$permissions->expects( $this->any() )
			->method( 'isAllowed' )
			->will( $this->returnValueMap( array(
				array( $row->revision, 'view', true ),
				array( $row->previousRevision, 'history', false ),
			) ) );
		...
		$this->assertEquals(
			mb_strlen( $nextContent ),
			$result['size']['new'],
			'Denying access to previous revision has no effect on this revisions size'
		);
		$this->assertEquals(
			null,
			$result['size']['old'],
			'With history permissions denied to previous revision a null size is output'
		);
	}

	public function generateRow( $plaintext = 'titlebar content' ) {
		$row = new FormatterRow;
		$row->workflow = Workflow::create( 'topic', $this->user, Title::newMainPage() );
		$row->rootPost = PostRevision::create( $row->workflow, $plaintext );
