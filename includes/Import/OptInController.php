...
class OptInController {
	...
	public function enable( Title $title, User $user ) {
		...
		$wikitextTalkpageArchiveTitle = null;
		...
		if ( $archivedFlowPage ) {
			...
			$this->createFlowBoard( $title );
		}

		// add archive template to board header
		if ( $wikitextTalkpageArchiveTitle ) {
			$linkToArchivedTalkpage = $this->buildLinkToArchivedTalkpage( $wikitextTalkpageArchiveTitle );
			$htmlHeaderPart = Utils::convert( 'wikitext', 'html', $linkToArchivedTalkpage, $title );
...
//		wfWaitForSlaves();
	 ...
	 * @param Title $title
	 */
	private function createFlowBoard( Title $title ) {
//		$loader = Container::get( 'factory.loader.workflow' )->createWorkflowLoader( $title );
		$workflow = Workflow::create( 'discussion', $title );
		$status = $this->occupationController->allowCreation( $title, $this->occupationController->getTalkpageManager() );
		if ( $status->isGood() ) {
			$this->occupationController->ensureFlowRevision( new Article( $title ), $workflow );
		}
	}
