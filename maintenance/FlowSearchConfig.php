...
class FlowSearchConfig extends Maintenance {
	...
	protected function getValidators() {
		...
		if ( $this->getOption( 'justCacheWarmers', false ) ) {
			// @todo: CacheWarmersValidator will need to be extended for Flow, has some $wgCirrus global in there...
			$validators[] = new CacheWarmersValidator( $this->indexType, $this->getTopicType(), $this );
			$validators[] = new CacheWarmersValidator( $this->indexType, $this->getHeaderType(), $this );
			return $validators;
		}
		...
		// @todo: probably need reindexer for both topic & header (as opposed to just PageType, for Cirrus)
		$reindexer = new Reindexer( $this->getIndex(), Connection::getSingleton(), $this->getTopicType(), $this->getOldTopicType(), $this->getShardCount(), $this->getReplicaCount(), $this->maintenanceTimeout, $this->getMergeSettings(), $this->getMappingConfig(), $this );
		...
		$validators[] = new SpecificAliasValidator( $this->getClient(), $this->getIndexName(), $this->getSpecificIndexName(), $this->startOver, $reindexer, $reindexParams, $reindexValidators, $this->reindexAndRemoveOk, $this->tooFewReplicas, $this );
...
//			$this->indexNamespaces(); // @todo: method doesn't yet exist
	...
	protected function pickAnalyzer( $langCode, array $availablePlugins = array() ) {
		// @todo: I guess Cirrus AnalysisConfigBuilder is ok, or do we need some Flow-specific extend?
