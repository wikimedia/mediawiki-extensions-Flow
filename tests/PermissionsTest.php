...
/**
 * Tests permissions for different types of users.
 *
 * This test assumes default permissions as defined in Flow.php
 * ($wgGroupPermissions) - if your config is different, these tests will no
 * longer be accurate.
 */
...
//			array( $this->oversightUser(), $this->header(), 'edit-header', true ),
	...
	public function testPermissions( User $user, PostRevision $revision = null, $action, $expected ) {
		...
		$this->assertEquals( $permissions->isAllowed( $revision, $action ), $expected );
	}
	...
	protected function blockedUser() {
		if ( !$this->blockedUser ) {
			$this->blockedUser = User::newFromName( 'UTBlockee' );
	...
	protected function unconfirmedUser() {
		if ( !$this->unconfirmedUser ) {
			$this->unconfirmedUser = User::newFromName( 'UTUnconfirmed' );
	...
	protected function confirmedUser() {
		if ( !$this->confirmedUser ) {
			$this->confirmedUser = User::newFromName( 'UTConfirmed' );
	...
	protected function sysopUser() {
		if ( !$this->sysopUser ) {
			$this->sysopUser = User::newFromName( 'UTSysop' );
	...
	protected function oversightUser() {
		if ( !$this->oversightUser ) {
			$this->oversightUser = User::newFromName( 'UTOversight' );
