...
' : '').'
    ...
    <div class="'.((LCRun3::ifvar($cx, ((is_array($in) && isset($in['isModerated'])) ? $in['isModerated'] : null))) ? ' flow-topic-moderated-titlebar' : '').' flow-topic-titlebar flow-click-interactive" data-flow-interactive-handler="topicCollapserToggle" tabindex="0">
	...
	'.((LCRun3::ifvar($cx, ((is_array($in) && isset($in['isTopicSummaryEditable'])) ? $in['isTopicSummaryEditable'] : null))) ? '
	...
	<span class="flow-ui-tooltip-triangle"></span>
		...
		<textarea id="flow-post-'.htmlentities(((is_array($in) && isset($in['postId'])) ? $in['postId'] : null), ENT_QUOTES, 'UTF-8').'-form-content"
				...
				class="mw-ui-input"
				type="text"
				placeholder="'.LCRun3::ch($cx, 'l10n', Array('flow-reply-topic-title-placeholder',((is_array($in) && isset($in['content'])) ? $in['content'] : null)), 'encq').'"
...
';}).'
	...
	<a data-flow-interactive-handler="apiRequest"
	   ...
	   class="flow-ui-button flow-ui-progressive /*flow-ui-quiet flow-ui-thin*/"><span class="wikiglyph wikiglyph-article"></span> '.LCRun3::ch($cx, 'l10n', Array('flow-load-more'), 'encq').'</a>
' : '').'
</div>
';
}
