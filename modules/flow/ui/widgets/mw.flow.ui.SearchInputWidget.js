( function ( $ ) {
	 ...
	 */
	mw.flow.ui.SearchInputWidget = function mwFlowUiSearchInputWidget( system, config ) {
		config = config || {};
	...
	mw.flow.ui.SearchInputWidget.prototype.onLookupMenuItemChoose = function ( item ) {
		...
		this.loadResultsToBoard( this.topiclist );
	...
	mw.flow.ui.SearchInputWidget.prototype.loadResultsToBoard = function ( topiclist ) {
		...
		// Rebuild the dm.Board
		dmBoard.clearItems();
		this.system.populateBoardTopicsFromJson( topiclist );

		// TODO2: Make sure infinite scroll works with search now
	};
