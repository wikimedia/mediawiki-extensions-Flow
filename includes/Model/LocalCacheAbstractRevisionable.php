...
abstract class LocalCacheAbstractRevisionable extends AbstractRevisionable {
	...
	public function getRevision( UUID $uuid ) {
		...
		while ( !$this->loaded() ) {
			...
			if ( $this->getStorage()->got( $previousId ) ) {
				...
				// needs to be added to beginning of array (so higest array
				// index is most recent post, lowest index is oldest)
				$this->revisions = array( $previousId->getAlphadecimal() => $revision ) + $this->revisions;
	...
	public function getLastRevision() {
		...
		if ( $this->storage->found( $attributes, $options ) ) {
			...
			$revision = $this->storage->find( $attributes, $options );
