...
class RevisionCollectionPermissionsTest extends PostRevisionTestCase {
	...
	public function testPermissions( User $user, $permisisonAction, $actions ) {
		...
		foreach ( $actions as $action ) {
			$action = key( $action );
			$actions[] = $action . ':' . ( current( $action ) ? 'true' : 'false' );
		...
		foreach ( $actions as $action ) {
			...
			$this->assertEquals(
				...
				'User ' . $user->getName() . ' should ' . ( $expected ? '' : 'not ' ) . 'be allowed action ' . $permisisonAction . ' on revision ' . key( $action ) . ' : ' . $actions . ' : ' . json_encode( $revision::toStorageRow( $revision ) )
			);
		}
	}
