...
class WorkflowLoader {
	...
	public function __construct(
		...
		if ( ! $workflow || ! $definition ) {
			throw new InvalidDataException( 'Unable to load workflow and definition', 'fail-load' );
		}
	...
	protected function loadWorkflow( \Title $title ) {
		...
		if ( !$definition->getOption( 'unique' ) ) {
			throw new InvalidDataException( 'Workflow is non-unique, can only fetch object by title + id', 'fail-load' );
		}
	...
	public function createBlocks( ) {
		...
		foreach ( $blocks as $block ) {
			if ( !isset( $return[$block->getName()] ) ) {
				...
				throw new InvalidDataException( 'Multiple blocks with same name is not yet supported', 'fail-load' );
			}
		}
	...
	public function handleSubmit( $action, array $blocks, $user, \WebRequest $request ) {
		...
		if ( !$interestedBlocks ) {
			if ( !$blocks ) {
				throw new InvalidDataException( 'No Blocks?!?', 'fail-load' );
			}
			$type = array();
			foreach ( $blocks as $block ) {
