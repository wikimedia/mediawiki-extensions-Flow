...
mw.flow.editor = {
				...
				new mw.Api()
					...
					.done( function( response ) {
						var content = response.query['flow-revision-content'].content,
							format = response.query['flow-revision-content'].format;

						// convert to editor's format
						content = mw.flow.parsoid.convert( format, mw.flow.editor.getFormat(), content );
	...
	getContent: function( $node ) {
		...
		return mw.flow.parsoid.convert( mw.flow.editor.getFormat(), 'html', content );
	},
