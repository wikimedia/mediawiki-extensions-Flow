...
class ConvertToText extends Maintenance {
	...
	public function execute() {
		...
		$topics = array();
		$headerContent = '';
		$headerContentFormat = 'wikitext';

		$headerData = $this->flowApi( $pageTitle, 'header-view', array( 'vhcontentFormat' => 'wikitext' ), 'header' );

		$headerRevision = $headerData['header']['revision'];
		if ( isset( $headerRevision['content'] ) ) {
			$headerContent = $headerRevision['content'];
			$headerContentFormat = $headerRevision['contentFormat'];
		} else {
			$headerContent = '';
		}
		...
		while( $continue ) {
			$continue = false;
			$flowData = $this->flowApi( $pageTitle, 'topiclist-view', $pagerParams, 'topiclist' );
	...
	public function processPostCollection( array $context, array $collection, $indentLevel = 0 ) {
		...
		foreach( $collection as $postId ) {
			...
			if ( $contentFormat !== 'wikitext' ) {
				$content = Utils::convert( $contentFormat, 'wikitext', $content );
