...
class XmlTopic {
	...
	public function getTitle() {
		...
		if ( $nodeList->length > 0 ) {
			...
			// In html mode the topic will always exist, even when
			// not provided by the document.
			$isNewNode = $node->getAttribute( 'data-flow' ) === '';
			return $this->title = new TitleNode( $this, $node, $isNewNode );
		}
		...
		if ( $nodeList->length > 0 ) {
			...
			return $this->title = new TitleNode( $this, $node, /* $isNewNode = */ true );
		}
	...
	/**
	 * @param UUID $id
	 * @return RevisionableNode|null
	 * @throws Exception on mal-formed document
	 */
	public function getRevision( UUID $id ) {
		...
		if ( $nodeList->length > 1 ) {
			wfDebugLog( 'Flow', __METHOD__ . ": Found multiple nodes for single revision $alpha" );
		}
