	...
	( function () {
		...
		function genModerateHandler( action, successCallback ) {
			return function ( info, data, jqxhr ) {
				if ( info.status !== 'done' ) {
					console.log( 'network level request failure, retry?' );
					return;
				}
				...
				if ( data.error ) {
					...
					// @todo display error
					console.log( 'top level api request failure, bad request?' );
					return;
				}

				if ( !data.flow[action] ) {
					console.log( 'bad request, nothing received for: ' + action );
				...
				if ( errors ) {
					...
					successCallback(
						$( this ).closest( 'form' ).data( 'flow-dialog-owner' ),
