...
class ApiBackend {
	...
	/**
	 * Retrieves data about a set of pages from the API
	 * @param  array  $titles Titles to return data for.
	 * @return array The query.pages part of the API response.
	 */
	public function retrievePageDataByTitle( array $titles ) {
		if ( !$titles ) {
	...
	/**
	 * Retrieves data about a set of pages from the API
	 * @param array $conds Conditions to retrieve pages by; to be sent to the API.
	 * @return array The query.pages part of the API response.
	 */
	public function retrievePageData( array $conds ) {
		$apiParams = $conds + array(
			'action' => 'query',
			'prop' => 'revisions',
		...
		if ( ! isset( $data['query'] ) ) {
			if ( $this->isNotFoundError( $data ) ) {
				throw new ApiNotFoundException( "Did not find pages: " . json_encode( $conds ) );
			} else {
				throw new ImportException( "Null response from API module", $data );
			}
		} elseif ( isset( $data['continue'] ) ) {
			wfWarn( "More revisions than can be retrieved for pageids " . json_encode( $pageids ) );
		}
