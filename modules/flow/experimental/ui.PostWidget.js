mw.flowExperimental.ui.PostWidget = function MwFlowPostWidget( id, config ) {
		...
		this.$element
			...
			.append(
				...
				$( '<div>' )
					...
					.append( config.postContent || 'Testing content' ),
				// Bottom menu
				$( '<div>' )
					.addClass( 'mw-flow-ui-table mw-flow-ui-postWidget-bottomMenu' )
					.append(
						$( '<div>' )
							.addClass( 'mw-flow-ui-row' )
	...
	this.topicTitle = config.$element ?
		...
		'';
	this.$bottomMenu = this.$element.children( '.mw-flow-ui-postWidget-bottomMenu' );

	// Take over elements
	this.setGroupElement( this.$element.children( '.mw-flow-ui-postWidget-replies' ) );
	...
	// Events
	this.replyButton.connect( this, { click: [ 'triggerAction', 'reply' ] } );
	this.thankButton.connect( this, { click: [ 'triggerAction', 'thank' ] } );
...
debugger;
		this.$element.after( this.replyWidget.$element );
...
mw.flowExperimental.ui.PostWidget.prototype.onSubmitReply = function () {
	var replyText = this.replyWidget.getInputValue();
