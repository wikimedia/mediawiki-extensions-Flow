...
class PageImportStateTest extends \MediaWikiTestCase {
	...
	protected function createState( $returnAll = false ) {
		...
		$state = new PageImportState( $workflow, $storage, new NullImportSourceStore() );
		if ( $returnAll ) {
	...
	public function testSetsRevIdAndPostIdForReplys() {
		...
		$topicTitle = PostRevision::create( $topicWorkflow, $user, 'sing song' );
		$reply = $topicTitle->reply( $topicWorkflow, $user, 'fantastic!' );
