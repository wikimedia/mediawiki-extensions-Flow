...
mw.flow.dm.System.prototype.populateBoardTopicsFromJson = function ( topiclist ) {
	...
	// Set offset Id
	this.topicProvider.setOffsetId( roots[ roots.length - 1 ] );
};
 ...
 *  flow.dm.Board
 */
mw.flow.dm.System.prototype.getMoreToCTopics = function () {
	var system = this;

	direction = direction || 'fwd';

	if ( offsetId ) {
		this.topicProvider.setOffsetId( offsetId );
	}
	...
	return this.topicQueue.get()
		.then( function ( data ) {
			...
			for ( postId in result ) {
				...
				// The 'unstub' should only go to topics that are really
