...
class ImportPost extends PageRevisionedObject implements IImportPost {
	...
	public function getAuthor() {
		if ( IP::isIPAddress( $this->apiResponse['author']['name'] ) ) {
			return User::newFromName( $this->apiResponse['author']['name'], false );
		} else {
			$user = User::newFromName( $this->apiResponse['author']['name'] );
			if ( !$user ) {
				throw new ImportException( 'Unable to create user: ' . $this->apiResponse['author']['name'] );
			}
			return $user;
		}
	}
...
class ImportRevision implements IObjectRevision {
	...
	public function getAuthor() {
		// @todo only pass false with valid ip address
		if ( IP::isIPAddress( $this->apiResponse['user'] ) ) {
			return User::newFromName( $this->apiResponse['user'], false );
		} else {
			$user = User::newFromName( $this->apiResponse['user'] );
			if ( !$user ) {
				throw new ImportException( 'Unable to create user: ' . $this->apiResponse['user'] );
			}
			return $user;
		}
	}
