...
class TopicBlock extends AbstractBlock {
	...
	public function render( Templating $templating, array $options, $return = false ) {
		...
		case 'compare-post-revisions':
			...
			$revisionView = PostRevisionView::newFromId( $options['newRevision'], $templating, 'PostRevision' );
			if ( !$revisionView ) {
			...
			if ( isset( $options['oldRevision'] ) ) {
				return $revisionView->compareAgainst( $options['oldRevision'], $this, $this->user, $return );
			} else {
				return $revisionView->compareAgainstPrevious( $this, $this->user, $return );
			}
			break;
		...
		default:
			...
			if ( !$this->permissions->isAllowed( $root, 'view' ) ) {
				...
				$revisionView = PostRevisionView::newFromId( $options['revId'], $templating, 'PostRevision' );
				if ( !$revisionView ) {
				...
				$revisionView->render( $this, $this->user, $return );
			} else {
