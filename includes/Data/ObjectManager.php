...
abstract class FeatureIndex implements Index {
	...
	public function findMulti( array $queries, array $options = array() ) {
		...
		foreach ( $cached as $key => $result ) {
			...
			foreach ( $indexes as $i ) {
				// write to complete results array (where the index of the result
				// matches the index of the given queries)
				$results[$i] = $result;
		...
		if ( $queries ) {
			$remaining = $this->backingStoreFindMulti( $queries, $this->queryOptions() );
	 ...
	 * basically, at that point, we only have a list of ids, which we need
	...
	protected function backingStoreFindMulti( array $queries, array $options = array() ) {
		// query backing store
