( function ( $ ) {
	...
	mw.flow.ui.EditorWidget = function mwFlowUiEditorWidget( config ) {
		...
		if ( mw.user.isAnon() ) {
			...
			this.anonWarning = new OO.ui.LabelWidget( {
				classes: [ 'flow-ui-editorWidget-anon-warning' ],
				label: $( $.parseHTML( mw.message( 'flow-anon-warning' ).params( [
		...
		this.$element
			.append(
				this.anonWarning && this.anonWarning.$element,
