		...
		switch( $eventName ) {
			case 'flow-post-reply':
				...
				$extraData += array(
					...
					'topic-title' => $this->language->truncate( trim( $topicRevision->getContent( 'topic-title-wikitext' ) ), 200 ),
				);
				$newPost = array(
					'title' => $title,
					'user' => $user,
			...
			case 'flow-topic-renamed':
				$extraData += array(
					'old-subject' => $this->language->truncate( trim( $topicRevision->getContent( 'topic-title-wikitext' ) ), 200 ),
					'new-subject' => $this->language->truncate( trim( $revision->getContent( 'topic-title-wikitext' ) ), 200 ),
				);
			break;
