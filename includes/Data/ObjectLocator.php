...
class ObjectLocator {
	 ...
	 * @return $queries Queries for BasicDbStorage class
	 */
	protected function convertToDbQueries( $queries, $options ) {
		if ( isset( $options['offset-id'] ) && $options['offset-id'] !== null &&
			isset( $options['sort'] ) && count( $options['sort'] ) === 1 &&
			preg_match( '/_id$/', $options['sort'][0] ) ) {
				if ( $options['offset-dir'] === 'fwd' ) {
					// Pagination same direction as within-page sort

					$operator = $options['order'] === 'DESC' ? '<' : '>';
				} else {
					// Pagination opposite direction from within-page sort
