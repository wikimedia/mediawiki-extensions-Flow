...
class PostBlock extends AbstractBlock {
	...
	public function render( Templating $templating, array $options, $return = false ) {
		...
		switch( $this->action ) {
			...
			default:
				...
				if ( isset( $options['id'] ) ) {
					$indexDescendant = $root->registerDescendant( $options['d'] );
					$post = $root->getRecursiveResult( $indexDescendant );
					if ( $post === false ) {
						throw new \MWException( 'Requested postId is not available within post tree' );
					}

					// @todo: correct post should've probably been loaded already (since workflow will no longer be topic, but the post), at this point; unsure how to deal with this yet;
					// @todo: should call ->render() on this correct $post
				}
	...
	public function renderAPI( Templating $templating, array $options ) {
		if ( isset( $options['id'] ) ) {
			...
			$indexDescendant = $rootPost->registerDescendant( $options['id'] );
