...
abstract class RevisionView {
	...
	protected function generateDiffViewTitle( AbstractRevision $revision ) {
		...
		$message = wfMessage( 'flow-compare-revisions-revision-header' )
			...
			->params( $this->templating->getUserText( $revision, $block->getUser() ) );
...
class HeaderRevisionView extends RevisionView {
	 ...
	 */
	public function __construct( Header $revision, Templating $templating, HeaderBlock $block ) {
	 ...
	 */
	public static function newFromId( $revId, Templating $templating, HeaderBlock $block ) {
		$revision = self::createRevision( $revId, $block->getStorage() );
	
		if ( $revision ) {
			return new self( $revision, $templating, $block );
		} else {
			return false;
		}
	}
...
class PostRevisionView extends RevisionView {
	 ...
	 */
	public function __construct( PostRevision $revision, Templating $templating, TopicBlock $block ) {
		parent::__construct( $templating );
		$this->block = $block;
		$this->revision = $revision;
	}
	 ...
	 */
	public static function newFromId( $revId, Templating $templating, TopicBlock $block ) {
		$revision = self::createRevision( $revId, $block->getStorage() );
