...
class TopicListBlock extends AbstractBlock {
	...
	protected function validate() {
		...
		$status = AbuseFilterUtils::validate( $this->topicPost, null, $this->workflow->getArticleTitle() );
		if ( !$status->isOK() ) {
...
//			return;
		...
		if ( $this->firstPost ) {
			// run through AbuseFilter
			$status = AbuseFilterUtils::validate( $this->firstPost, null, $this->workflow->getArticleTitle() );
			if ( !$status->isOK() ) {
				foreach ( $status->getErrorsArray() as $message ) {
					$this->addError( 'abusefilter', wfMessage( array_shift( $message ), $message ) );
				}
