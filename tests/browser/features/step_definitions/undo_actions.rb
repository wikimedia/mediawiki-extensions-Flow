...
Given(/^I am on a new Flow board with description "(.*?)"$/) do |content|
  @boardTitle = @data_manager.get 'Board_for_undo_actions'
  api.action('flow',
    submodule: 'edit-header',
    page: 'Flow_test_talk:New_page_' + @boardTitle,
    ehcontent: content,
    ehformat: 'wikitext')
  visit(NewFlowPage, using_params: { pagetitle: @boardTitle })
end

Given(/^I am on a new Flow board with topic content \"(.*?)\"$/) do |content|
  @boardTitle = @data_manager.get 'Board_for_undo_actions'
  @topicTitle = @data_manager.get 'Title_for_undo_actions'
  api.action('flow',
    submodule: 'new-topic',
    page: 'Flow_test_talk:New_page_' + @boardTitle,
    nttopic: @topicTitle,
    ntcontent: content)
  step "I am on a Flow board \"#{@boardTitle}\""
  on(NewFlowPage) do |page|
    page.refresh_until { page.topic_with_title(@topicTitle) }
  end
end

When(/^I visit the new board history page$/) do
  visit(BoardHistoryPage, using_params: { pagetitle: 'Flow_test_talk:New_page_' + @boardTitle })
