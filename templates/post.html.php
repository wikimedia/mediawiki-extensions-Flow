	...
	echo Html::openElement( 'div', array(
		...
		<div class="flow-post-main">
			...
			<div class="flow-post-interaction">
				<?php if ( !$post->isModerated() ): ?>
					<a class="flow-reply-link mw-ui-button" href="#"><span><?php echo wfMessage( 'flow-reply-link' )->escaped(); ?></span></a>
					<a class="flow-thank-link mw-ui-button" href="#" onclick="alert( '@todo: Not yet implemented!' ); return false;"><span><?php echo wfMessage( 'flow-thank-link' )->escaped(); ?></span></a>
				<?php else: ?>
					<?php
						$user = User::newFromId( $post->getUserId() );
						$title = $user->getTalkPage();
					?>
					<a class="flow-talk-link mw-ui-button" href="<?php echo $title->getLinkURL(); ?>">
						<span><?php echo wfMessage( 'flow-talk-link', $user->getName() )->escaped(); ?></span>
					</a>
				<?php endif; ?>
			</div>
		</div>
