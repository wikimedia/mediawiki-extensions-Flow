...
abstract class AbstractRevision {
	...
	/**
	 * @var UUID The revision prior to this one, or null if this is first revision
	 */
	...
	/**
	 * @var string Only populated when external store is in use
	 */
	protected $contentUrl;

	/**
	 * @var string This is decompressed on-demand from $this->content in self::getContent()
	 */
	...
	public function getContent( $format = 'html' ) {
		...
		if ( $this->xssCheck === null && $sourceFormat === 'html' ) {
			...
			if ( !$this->xssCheck ) {
				wfDebugLog( __CLASS__, __FUNCTION__ . ': XSS check prevented display of revision ' . $this->revId->getAlphadecimal() );
				return '';
			}
		}
