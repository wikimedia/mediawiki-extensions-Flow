...
( function ( $, mw ) {
	 ...
	 * @return {FlowBoardComponentApiEventsMixin}
	...
	FlowBoardComponentApiEventsMixin.UI.events.apiHandlers.moderateTopic = _genModerateHandler(
		...
		function ( $target, revision ) {
			...
			flowBoard.emitWithReturn( 'makeContentInteractive', $titlebar );
		}
	);
	...
	if ( inTopicNamespace ) {
		...
		FlowBoardComponentApiEventsMixin.UI.events.interactiveHandlers.collapserGroupToggle = $.noop;
		FlowBoardComponentApiEventsMixin.UI.events.interactiveHandlers.collapserCollapsibleToggle = $.noop;
	}

	// Extend FlowBoardComponent
	mw.flow.extendComponent( 'board', FlowBoardComponentApiEventsMixin );
}( jQuery, mediaWiki ) );