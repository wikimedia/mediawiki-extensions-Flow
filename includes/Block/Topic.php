	...
	public function renderApi( array $options ) {
		...
		switch ( $this->action ) {
			...
			case 'view-post-history':
				...
				$output += $this->renderHistoryApi( Container::get( 'query.post.history' ), UUID::create( $options['postId'] ), $options, false );
				break;

			case 'view-topic-history':
				// View entire history of a topic's posts
				$output += $this->renderHistoryApi( Container::get( 'query.topic.history' ), $this->workflow->getId(), $options, false );
				break;
