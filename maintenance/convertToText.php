...
class ConvertToText extends Maintenance {
	...
	private function processHeader() {
		return $this->processMultiRevisions( $this->getAllRevisions( $this->pageTitle, 'view-header', 'vh', 'header' ) );
	}
	private function processMultiRevisions( $allRevisions ) {
		if ( count( $allRevisions ) ) {
			...
			$latestContributor = $latestRevision['author'];

			// deduplicate authors (todo: keep IPs?)
			$previousContributors = [];
			foreach ( $allRevisions as $revision ) {
				$name = $revision['author']['name'];
				$previousContributors[ $name ] = $revision['author'];
			}

			// remove latest contributor from list of previous contributors
			if ( isset( $previousContributors[ $latestContributor['name'] ] ) ) {
				unset( $previousContributors[ $latestContributor['name'] ] );
			}

			$formattedAuthors = $this->getSignature( $latestContributor, $latestRevision['timestamp'] );

			if ( count( $previousContributors ) ) {
				$signatures = array_map( array( $this, 'getSignature' ), $previousContributors );
				$formattedAuthors .= ' ( ' . implode( ', ', $signatures ) . ' )';
			}

			return $content . ' ' . $formattedAuthors;
		}
		return '';
	}
	...
	private function processPost( $revision ) {
		...
		$allPostRevisions = $response['topic']['revisions'];
		return $this->processMultiRevisions( $allPostRevisions );
	}
