	...
	( function () {
		...
		FlowBoardComponent.UI.events.interactiveHandlers.showEditHeaderForm = function ( event ) {
			...
			if ( flowId ) {
				...
				$deferred.done ( function( result ) {
						$( self ).closest( '.flow-board-header' ).replaceWith(
							mw.flow.TemplateEngine.processTemplate( 'flow_block_header_edit', result.flow['header-view'].result.header )
						);
					}
				).fail( function () {
		...
		FlowBoardComponent.UI.events.interactiveHandlers.submitHeader = function ( event ) {
			...
			api.post( params ).done( function ( result ) {
				$( self ).closest( '.flow-board-header' ).replaceWith(
					mw.flow.TemplateEngine.processTemplate( 'flow_block_header', result.flow['edit-header'].result.header )
				);
			} ).fail( function () {
				alert( '@Todo: replace with error handling');
			} );
