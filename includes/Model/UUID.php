...
class UUID {
	...
	/**
	 * binary UUID encoded for database storage
	 *
	 * @var Blob|string
	 */
	...
	public function getBinary() {
		if ( $this->binaryValue !== null ) {
			...
			$binaryValue = static::hex2bin( $this->hexValue );
		...
		foreach( $array as $key => $value ) {
			if ( $value instanceof Blob ) {
				...
				if ( $format === 'alphadecimal' && $len === self::BIN_LEN ) {
					...
					// for the database, that is unhandled here.  A patch for is under
