...
if ( $revision->getPrevRevisionId() ) {
	$compareLink = $urlGenerator->generateUrl(
		...
		'compare-revisions',
		array(
			$block->getName().'[newRevision]' => $revision->getRevisionId()->getAlphadecimal(),
			$block->getName().'[oldRevision]' => $revision->getPrevRevisionId()->getAlphadecimal()
		)
	);
} else {
	$compareLink = false;
...
switch( $revision->getRevisionType() ) {
	case 'post':
		$historyLink = $urlGenerator->generateUrl(
			...
			'post-history',
			array(
				$block->getName().'[postId]' => $revision->getPostId()->getAlphadecimal(),
			)
		);
