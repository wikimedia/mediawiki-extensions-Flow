...
class TreeRepository {
	...
	public function findRootPaths( array $descendants ) {
		...
		foreach( $descendants as $descendant ) {
			...
			$path = $this->cache->get( $cacheKeys[$descendant->getHex()] );
			if ( $path !== false ) {
				$cacheValues[$descendant->getHex()] = $path;
			} else {
		...
		$res = $dbr->select(
			...
			array(
				'tree_descendant_id' => array_values( $missingValues ),
			),
			__METHOD__
		);
	...
	public function findRoots( array $descendants ) {
		...
		foreach( $descendants as $descendant ) {
			if ( isset( $paths[$descendant->getHex()] ) ) {
				$roots[$descendant->getHex()] = array_shift( $paths[$descendant->getHex()] );
			}
		}
