...
( function ( $, mw ) {
	...
	function _flowBoardAdjustTopicNavigationHeader( $boardNavigation, event, extraParameters ) {
		var bottomScrollPosition, topicText, newReadingTopicId, navBarRect, width, left,
			self = this,
		...
		left = parseInt( $boardNavigation.css( 'left' ) );
		width = parseInt( $boardNavigation.css( 'width' ) );
		if ( left !== boardNavigationPosition.left || width !== navBarRect.width ) {
			$boardNavigation.css( {
