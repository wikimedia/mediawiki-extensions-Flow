	...
	public function render( Templating $templating, array $options, $return = false ) {
		...
		switch ( $this->action ) {
			case 'compare-header-revisions':
				...
				$revisionView = HeaderRevisionView::newFromId( $options['newRevision'], $templating, 'Header' );
				if ( !$revisionView ) {
			...
			default:
				if ( isset( $options['revId'] ) ) {
					$revisionView = HeaderRevisionView::newFromId( $options['revId'], $templating, 'Header' );
					if ( !$revisionView ) {
