...
( function ( mw, $ ) {
	...
	function FlowEventLog( schemaName, eventInstance ) {
		...
		function logEvent( eventInstance ) {
			// Merge data for this specific event with already known data
			eventInstance = $.extend( this.eventInstance, eventInstance );

			if ( mw.eventLog && mw.config.get( 'wgFlowLogEvents' ) ) {
