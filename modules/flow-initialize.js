...
( function ( $ ) {
	...
	$( document ).ready( function () {
		...
		descriptionWidget = new mw.flow.ui.BoardDescriptionWidget( dmBoard.getDescription(), {
			$existing: $( '.flow-ui-boardDescriptionWidget-content' ).html()
		} );
		...
		function replaceReplyForms( $element ) {
			$element.find( '.flow-post.flow-reply-form' ).each( function () {
				...
				editorWidget.on( 'saveContent', function ( content, contentFormat ) {
					editorWidget.editorSwitcherWidget.pushPending(); // FIXME shouldn't have to reach in
					...
					new mw.Api().postWithToken( 'edit', {
						...
						.then( function () {
							// Destroy the editor
							editorWidget.editorSwitcherWidget.popPending();
