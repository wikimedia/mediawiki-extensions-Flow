...
class SpecialFlow extends FormSpecialPage {
	...
	protected function getFormFields() {
		return array(
			'type' => array(
				'id' => 'mw-special-flow-type',
				'name' => 'type',
				'type' => 'select',
				'label-message' => 'special-flow-type',
				'options' => $this->getTypes(),
				'default' => empty( $this->type ) ? 'post' : $this->type,
			),
			'uuid' => array(
				'id' => 'mw-special-flow-uuid',
				'name' => 'uuid',
				'type' => 'text',
				'label-message' => 'special-flow-uuid',
				'default' => $this->uuid,
			),
		);
	}
	...
	protected function preText() {
		return '<p>' . $this->msg( 'special-flow-desc' )->escaped() . '</p>';
	}
	...
	public function onSubmit( array $data ) {
		...
		if ( $url ) {
			...
			return Status::newFatal( 'special-flow-invalid-uuid' );
		}
	}
