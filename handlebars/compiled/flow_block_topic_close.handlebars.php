<?php return function ($in, $debugopt = 1) {
	...
	<form class="flow-edit-form" data-flow-initial-state="collapsed" method="POST"
		action="'.((LCRun3::ifvar($cx, ((is_array($in['revision']) && isset($in['revision']['isModerated'])) ? $in['revision']['isModerated'] : null))) ? '
				'.htmlentities(((is_array($in['revision']['actions']['reopen']) && isset($in['revision']['actions']['reopen']['url'])) ? $in['revision']['actions']['reopen']['url'] : null), ENT_QUOTES, 'UTF-8').'
			' : '
				'.htmlentities(((is_array($in['revision']['actions']['close']) && isset($in['revision']['actions']['close']['url'])) ? $in['revision']['actions']['close']['url'] : null), ENT_QUOTES, 'UTF-8').'
			').'">
		...
		<div class="flow-form-actions flow-form-collapsible">
			<button data-role="submit" class="flow-ui-button flow-ui-constructive">'.LCRun3::ch($cx, 'l10n', Array('flow-topic-action-close-topic'), 'encq').'</button>
