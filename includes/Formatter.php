...
abstract class AbstractFormatter {
	 ...
	 *
	 * @param array $revisions Multi-dimensional array of revisions to fetch,
	...
	public function loadRevisions( array $revisions ) {
		...
		foreach ( $revisions as $revisionType => $revisionIds ) {
			// make sure all ids are UUID objects
			$revisionIds = array_map( array( 'Flow\Model\UUID', 'create' ), $revisionIds );

			foreach ( $revisionIds as $i => $revisionId ) {
				// don't query for revisions already in cache
				if ( isset( $this->revisions[$revisionId->getHex()] ) ) {
					$results[$revisionId->getHex()] = $this->revisions[$revisionId->getHex()];
					unset( $revisionIds[$i] );
				}
			}

			// fetch missing revisions
			$revisions = (array) $this->storage->getMulti( $revisionType, $revisionIds );
