	...
	$handlers = array(
		...
		new Flow\Data\WatchTopicListener(
			...
			array(
				...
				'reply' => $user->isAnon() ? array() : array( $user->getId() ),
				'edit' => $user->isAnon() ? array() : array( $user->getId() ),
				'edit-title' => $user->isAnon() ? array() : array( $user->getId() ),
				// More complex: callback will return array of users watching the board
				'new-post' => array( 'Flow\\Data\\WatchTopicListener', 'getUserIdsWatchingBoard' ),
			)
		),
		$c['collection.cache'],
		// topic history -- to keep a history by topic we have to know what topic every post
