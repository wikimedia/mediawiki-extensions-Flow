...
class TemplateHelper {
	...
	static public function l10n( $str /*, $args... */ ) {
		...
		if ( $message ) {
			return self::html( $message->text() );
		} else {
			static $cache;
			if ( !isset( $cache[$str] ) ) {
				$cache[$str] = self::html( wfMessage( $str )->text() );
			}
			return $cache[$str];
		}
	}
	...
	static public function eachPost( $context, $postIds, $options ) {
		...
		if ( $postIds && !is_array($postIds) ) {
			...
			return $inverse && $inverse( $options['cx'], array() );
		} else {
			return null;
		}
		...
		foreach ( $postIds as $id ) {
			...
			$cx['scopes'][] = $context['revisions'][$revId];
