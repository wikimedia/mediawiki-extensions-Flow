...
class UUID {
		...
		if ( is_string( $input ) || is_int( $input) || $input === false ) {
			if ( $input === false ) {
				...
				if ( $len === self::BIN_LEN ) {
					...
					return new static( $binaryValue, static::INPUT_BIN );
	...
	public function getBinary() {
		if ( $this->binaryValue !== null ) {
			...
			return $this->binaryValue = static::hex2bin( $this->hexValue );
	...
	protected function getHex() {
		if ( $this->hexValue !== null ) {
			...
			return $this->hexValue = static::bin2hex( $this->binaryValue );
		} elseif ( $this->alphadecimalValue !== null ) {
			return $this->hexValue = static::alnum2hex( $this->alphadecimalValue );
		}
	}
	...
	public function getAlphadecimal() {
		if ( $this->alphadecimalValue !== null ) {
			...
			return $this->alphadecimalValue = static::hex2alnum( $this->hexValue );
		} elseif ( $this->binaryValue !== null ) {
