...
class Templating {
	...
	public function printParticipants( $post ) {
		...
		while(
			...
			$text = $this->getUserText( $participant, null, false );
	 ...
	 */
	public function getUserText( $user, $rev = null, $showIPs = true ) {
		if ( $user === false && $rev instanceof AbstractRevision ) {
			...
			return wfMessage(
					AbstractRevision::$perms[$this->moderationState]['content'],
