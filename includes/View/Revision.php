...
abstract class RevisionView {
	 ...
	 */
	public function __construct( Templating $templating, AbstractBlock $block ) {
	...
	public function getDiffLinkAgainstPrevious() {
		$currentRevision = $this->getRevision();
		if ( $currentRevision->getPrevRevisionId() ) {
			$params = array(
				$this->block->getName().'_newRevision' => $currentRevision->getRevisionId()->getAlphadecimal(),
				$this->block->getName().'_oldRevision' => $currentRevision->getPrevRevisionId()->getAlphadecimal()
			);
...
class HeaderRevisionView extends RevisionView {
	 ...
	 * @param \Flow\Block\AbstractBlock $block
	 */
	public function __construct( Header $revision, Templating $templating, AbstractBlock $block ) {
		parent::__construct( $templating, $block );
...
class PostRevisionView extends RevisionView {
	 ...
	 * @param \Flow\Block\AbstractBlock $block
	 */
	public function __construct( PostRevision $revision, Templating $templating, AbstractBlock $block ) {
		parent::__construct( $templating, $block );
		$this->revision = $revision;
	}
