...
'.$sp.'' : '').'</div>
';},
'flow_moderate_topic' => function ($cx, $in, $sp) {return ''.$sp.'<form method="POST" action="'.LR::encq($cx, LR::hbch($cx, 'moderationAction', array(array(((isset($in['actions']) && is_array($in)) ? $in['actions'] : null),((isset($cx['sp_vars']['root']['submitted']['moderationState']) && is_array($cx['sp_vars']['root']['submitted'])) ? $cx['sp_vars']['root']['submitted']['moderationState'] : null)),array()), 'encq', $in)).'">
...
'.$sp.'	          placeholder="'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array(LR::hbch($cx, 'concat', array(array('flow-moderation-placeholder-',((isset($cx['sp_vars']['root']['submitted']['moderationState']) && is_array($cx['sp_vars']['root']['submitted'])) ? $cx['sp_vars']['root']['submitted']['moderationState'] : null),'-topic'),array()), 'raw', $in)),array()), 'encq', $in)).'"
...
'.$sp.'		        data-role="submit">'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array(LR::hbch($cx, 'concat', array(array('flow-moderation-confirm-',((isset($cx['sp_vars']['root']['submitted']['moderationState']) && is_array($cx['sp_vars']['root']['submitted'])) ? $cx['sp_vars']['root']['submitted']['moderationState'] : null),'-topic'),array()), 'raw', $in)),array()), 'encq', $in)).'</button>
...
'.$sp.'' : '').''.((LR::ifvar($cx, ((isset($in['name']) && is_array($in)) ? $in['name'] : null), false)) ? '<bdi>'.LR::encq($cx, ((isset($in['name']) && is_array($in)) ? $in['name'] : null)).'</bdi>' : ''.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-anonymous'),array()), 'encq', $in)).'').''.((LR::ifvar($cx, ((isset($in['links']['userpage']) && is_array($in['links'])) ? $in['links']['userpage'] : null), false)) ? '</a>' : '').'<span class="mw-usertoollinks flow-pipelist">
...
'.$sp.'			</a>
'.$sp.'' : '			<span>
'.$sp.''.LR::hbch($cx, 'ifCond', array(array(((isset($in['creator']['name']) && is_array($in['creator'])) ? $in['creator']['name'] : null),'===',((isset($in['lastEditUser']['name']) && is_array($in['lastEditUser'])) ? $in['lastEditUser']['name'] : null)),array()), $in, false, function($cx, $in)use($sp){return '					'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-edited'),array()), 'encq', $in)).'
'.$sp.'';}, function($cx, $in)use($sp){return '					'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-edited-by',((isset($in['lastEditUser']['name']) && is_array($in['lastEditUser'])) ? $in['lastEditUser']['name'] : null)),array()), 'encq', $in)).'
'.$sp.'';}).'			</span>
'.$sp.'			<a href="'.LR::encq($cx, ((isset($in['links']['diff-prev']['url']) && is_array($in['links']['diff-prev'])) ? $in['links']['diff-prev']['url'] : null)).'" class="flow-timestamp-anchor">'.LR::encq($cx, LR::hbch($cx, 'uuidTimestamp', array(array(((isset($in['lastEditId']) && is_array($in)) ? $in['lastEditId'] : null)),array()), 'encq', $in)).'</a>
'.$sp.'').'	</span>
'.$sp.'</div>
';},
...
'.$sp.'		'.LR::encq($cx, LR::hbch($cx, 'escapeContent', array(array(((isset($in['content']['format']) && is_array($in['content'])) ? $in['content']['format'] : null),((isset($in['content']['content']) && is_array($in['content'])) ? $in['content']['content'] : null)),array()), 'encq', $in)).'
'.$sp.'	</div>
'.$sp.'
...
'.$sp.''.LR::p($cx, 'flow_form_cancel_button', array(array($in),array()), '		').'		<small class="flow-terms-of-use plainlinks">'.LR::encq($cx, LR::hbch($cx, 'l10nParse', array(array('flow-terms-of-use-edit'),array()), 'encq', $in)).'</small>
'.$sp.'	</div>
'.$sp.'</form>
';},
...
'.$sp.'			'.LR::encq($cx, LR::hbch($cx, 'post', array(array(((isset($cx['sp_vars']['root']['rootBlock']) && is_array($cx['sp_vars']['root'])) ? $cx['sp_vars']['root']['rootBlock'] : null),$in),array()), 'encq', $in)).'
...
'.LR::sec($cx, ((isset($in['roots']) && is_array($in)) ? $in['roots'] : null), null, $in, true, function($cx, $in) {return ''.LR::hbch($cx, 'eachPost', array(array(((isset($cx['sp_vars']['root']) && is_array($cx['sp_vars'])) ? $cx['sp_vars']['root'] : null),$in),array()), $in, false, function($cx, $in) {return ''.LR::p($cx, 'flow_moderate_topic', array(array($in),array()), '			').''.LR::p($cx, 'flow_post_partial', array(array($in),array()), '			').'';}).'';}).'</div>
';
};
?>