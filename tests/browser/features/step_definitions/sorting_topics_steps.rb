...
Then(/^the Flow page should not show Recently active topics link$/) do
  on(FlowPage) do |page|
    page.recently_active_topics_link_element.when_not_visible
    expect(page.recently_active_topics_link_element).not_to be_visible
  end
end
...
Then(/^the Flow page should not show Newest topics link$/) do
  on(FlowPage) do |page|
    page.newest_topics_link_element.when_not_visible
