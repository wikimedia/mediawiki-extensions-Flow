...
';}).'
	...
	<div class="flow-post'.LCRun3::ifv($cx, ((is_array($in) && isset($in['isModerated'])) ? $in['isModerated'] : null), $in, function($cx, $in) {return ' flow-post-moderated';}).'" data-flow-id="'.htmlentities(((is_array($in) && isset($in['postId'])) ? $in['postId'] : null), ENT_QUOTES, 'UTF-8').'">
		...
		<div class="flow-menu">
			...
			<ul class="flow-ui-button-container">
				...
				';}).'
				'.LCRun3::ifv($cx, ((is_array($in['actions']) && isset($in['actions']['restore'])) ? $in['actions']['restore'] : null), $in, function($cx, $in) {return '
					<li>
						<a class="flow-ui-button flow-ui-progressive flow-ui-quiet flow-ui-thin"
