...
class UUID {
	...
	protected function __construct( $value, $format ) {
		...
		if ( is_string( $value ) && !ctype_digit( $value ) && $value !== strtolower( $value ) ) {
	...
	public function __wakeup() {
		...
		if ( $this->alphadecimalValue && strtolower( $this->alphadecimalValue ) !== $this->alphadecimalValue ) {
			// There was a bug writing invalid uuid's into cache.  We added code in the constructor
			// to prevent them from being created, but since this is coming from cache lets just
			// fix them and move on with the request
