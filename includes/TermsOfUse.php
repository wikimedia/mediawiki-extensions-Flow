...
class TermsOfUse {
	 ...
	 * @var string
	 */
	private static $restoreTopicTerms;
	...
	private static function setTerms() {
		...
		$restoreTopicKey = 'flow-terms-of-use-restore-topic';
		...
		wfRunHooks( 'FlowTermsOfUse', array( &$addTopicKey, &$replyKey, &$editKey, &$summarizeKey, &$closeTopicKey, &$restoreTopicKey ) );
		...
		self::$restoreTopicTerms = wfMessage( $restoreTopicKey )->parse();
	}
	 ...
	 * @return string
	 */
	public static function getRestoreTopicTerms() {
		if ( self::$restoreTopicTerms === null ) {
			self::setTerms();
		}
		return self::$restoreTopicTerms;
	}
