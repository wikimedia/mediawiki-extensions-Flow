...
class TopicBlock extends AbstractBlock {
	...
	protected function validateEditTopicSummary() {
		...
		// * The topic title doesn't exist, which should throw exception
		// * The user doesn't have permission to view this topic title, which is added to $this->error
		// Both are handled in loadTopicTitle(), nothing needes to be done in here
		if ( !$topicTitle ) {
			return;
		}
		...
		if ( !$summary ) {
			$this->postSummary = PostSummary::create( $topicTitle, $this->user, $this->submitted['summary'], 'create-topic-summary' );
		} else {
			$summary = reset( $summary );
			$this->postSummary = $summary->newNextRevision( $this->user, $this->submitted['summary'], 'edit-topic-summary' );
		}
	}
	...
	public function commit() {
		...
		case 'edit-topic-summary':
			...
			$this->storage->put( $this->postSummary );
			$newRevision = $this->postSummary;
			return array(
				'new-revision-id' => $this->postSummary->getRevisionId(),
