...
( function ( $, mw ) {
	...
	function initFlowComponent( $container ) {
		...
		if ( _componentRegistry.lookup( componentName ) ) {
			// Return the new instance of that FlowComponent, via our _RecursiveConstructor method
			OO.inheritClass( _RecursiveConstructor, _componentRegistry.lookup( componentName ) );
	 ...
	 * @param {String} [prototypeName='component']
	 */
	 ...
	 * Remember to correctly .call( this ) or .apply( this ) when necessary, to set the right context.
	 ...
	 */
	function extendFlowComponent( name, mixinClass ) {
		var registeredClass = _componentRegistry.lookup( name );
	...
	mw.flow.extendComponent = extendFlowComponent;
}( jQuery, mediaWiki ) );