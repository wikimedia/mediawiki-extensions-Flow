...
'.$sp.'' : '').'</div>
';},
'flow_moderate_post' => function ($cx, $in, $sp) {return ''.$sp.'<form method="POST" action="'.LR::encq($cx, LR::hbch($cx, 'moderationAction', array(array(((is_array($in) && isset($in['actions'])) ? $in['actions'] : null),(isset($cx['sp_vars']['root']['submitted']['moderationState']) ? $cx['sp_vars']['root']['submitted']['moderationState'] : null)),array()), 'encq', $in)).'">
...
'.LR::hbbch($cx, 'eachPost', array(array((isset($cx['sp_vars']['root']) ? $cx['sp_vars']['root'] : null),(isset($cx['sp_vars']['root']['submitted']['postId']) ? $cx['sp_vars']['root']['submitted']['postId'] : null)),array()), $in, false, function($cx, $in) {return ''.LR::p($cx, 'flow_moderate_post', array(array($in),array()),0, '		').''.LR::p($cx, 'flow_post_partial', array(array($in),array()),0, '		').'';}).'</div>
';
};