...
( function ( $, mw ) {
	...
	( function () {
		...
		FlowBoardComponent.UI.events.apiHandlers.preview = function( info, data, jqxhr ) {
			...
			templateParams = {
				author: {
					// @todo output IP address for anonymous users
			...
			$previewContainer
				.addClass( 'flow-preview' )
				.addClass( 'flow-ui-tooltip-target' ).attr( 'title', flowBoard.TemplateEngine.l10n('flow-viewing-preview') );

			$target.hide();
			$target.before( $previewContainer );
			...
			$button
				...
				.text( flowBoard.TemplateEngine.l10n('flow-edit') );
		};
		...
		function flowBoardComponentResetPreview( $cancelButton ) {
			...
			if ( oldData ) {
				...
				$target = $target && $target.length ? $target : $button;
