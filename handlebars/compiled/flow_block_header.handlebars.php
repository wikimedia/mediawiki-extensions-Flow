...
'.$sp.'</h2>
';},
'flow_header_edit_restrictions' => function ($cx, $in, $sp) {return ''.$sp.''.((!LR::ifvar($cx, ((isset($in['revision']['actions']) && is_array($in['revision']['actions']) && isset($in['revision']['actions']['edit'])) ? $in['revision']['actions']['edit'] : null), false)) ? '	<p class="flow-board-header-restricted">
...
'.$sp.'</a>
';},
'flow_header_categories' => function ($cx, $in, $sp) {return ''.$sp.''.((LR::ifvar($cx, (isset($cx['sp_vars']['root']['categories']['items']) ? $cx['sp_vars']['root']['categories']['items'] : null), false)) ? '<div id="catlinks" class="catlinks flow-board-header-category-view-nojs">
'.$sp.'	<div id="mw-normal-catlinks" class="mw-normal-catlinks">'.LR::encq($cx, LR::hbch($cx, 'html', array(array((isset($cx['sp_vars']['root']['categories']['link']) ? $cx['sp_vars']['root']['categories']['link'] : null)),array()), 'encq', $in)).'<ul class="flow-board-header-category-list">'.LR::sec($cx, (isset($cx['sp_vars']['root']['categories']['items']) ? $cx['sp_vars']['root']['categories']['items'] : null), null, $in, true, function($cx, $in)use($sp){return '<li class="flow-board-header-category-item">'.LR::encq($cx, LR::hbch($cx, 'html', array(array($in),array()), 'encq', $in)).'</li>';}).'</ul>
'.$sp.'	</div>
'.$sp.'</div>
'.$sp.'' : '').'';},
...
'.$sp.'  <hr />
'.$sp.'  <p>
'.$sp.'    '.LR::encq($cx, LR::hbch($cx, 'html', array(array(((is_array($in) && isset($in['copyrightMessage'])) ? $in['copyrightMessage'] : null)),array()), 'encq', $in)).'
'.$sp.'  </p>
'.$sp.'</div>
';});
    $cx = array(
