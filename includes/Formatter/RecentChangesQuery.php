...
class RecentChangesQuery extends AbstractQuery {
	...
	public function loadMetadataBatch( $rows, $isWatchlist = false ) {
		...
		foreach ( $rows as $row ) {
			...
			if ( !$params ) {
				wfDebugLog( __CLASS__, __FUNCTION__ . ": rc_params does not contain serialized content: {$row->rc_params}" );
				continue;
			}
	...
	public function getResult( ChangesList $cl, RecentChange $rc, $isWatchlist = false ) {
		...
		if ( $isWatchlist && $this->hideRecord( $changeData ) ) {
			return false;
		}
	 ...
	 */
	protected function hideRecord( array $changeData ) {
