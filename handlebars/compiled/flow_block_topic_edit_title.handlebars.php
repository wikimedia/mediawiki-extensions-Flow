...
'.$sp.'' : '').'</div>
';},
'flow_edit_topic_title' => function ($cx, $in, $sp) {return ''.$sp.'<form method="POST" action="'.LR::encq($cx, ((isset($in['actions']['edit']) && is_array($in['actions']['edit']) && isset($in['actions']['edit']['url'])) ? $in['actions']['edit']['url'] : null)).'" class="flow-edit-title-form">
...
'.LR::sec($cx, ((is_array($in) && isset($in['roots'])) ? $in['roots'] : null), null, $in, true, function($cx, $in) {return ''.LR::hbbch($cx, 'eachPost', array(array((isset($cx['sp_vars']['root']) ? $cx['sp_vars']['root'] : null),$in),array()), $in, false, function($cx, $in) {return ''.LR::p($cx, 'flow_edit_topic_title', array(array($in),array()),0, '			').'';}).'';}).'</div>
';
};