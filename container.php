...
$c['storage.header'] = $c->share( function( $c ) {
	...
	$handlers = array(
		...
		new Flow\Data\ReferenceRecorder(
			$c['extractor.reference'],
...
$c['extractor.reference'] = $c->share( function( $c ) {
		...
		array(
			'//*[starts-with(@typeof, "mw:Image")]' =>
				function( $element ) use ( $stripTitle ) {
					...
					$imageLink = $imgNode->getAttribute( 'resource' );
					return array(
						'refType' => 'file',
						'targetType' => 'wiki',
			...
			'//a[@rel="mw:WikiLink"]' =>
				function( $element ) use ( $stripTitle ) {
					$linkTarget = $element->getAttribute( 'href' );
					return array(
						'refType' => 'link',
						'targetType' => 'wiki',
...
$c['reference.updater.links-tables'] = $c->share( function( $c ) {
	return new Flow\LinksTableUpdater( $c['storage'], $c['reference.table_config'], wfGetLB() );
} );

$c['reference.table_config'] = $c->share( function( $c ) {
	return array(
		'pagelinks' => array(
			'src_id' => 'pl_from',
			'namespace' => 'pl_namespace',
			'title' => 'pl_title',
			'_indexes' => array( 'pl_from', 'pl_namespace', 'pl_title' ),
		),
		'imagelinks' => array(
			'src_id' => 'il_from',
			'title' => 'il_to',
			'_indexes' => array( 'il_from', 'il_to' ),
		),
		'templatelinks' => array(
			'src_id' => 'tl_from',
			'namespace' => 'tl_namespace',
			'title' => 'tl_title',
			'_indexes' => array( 'tl_from', 'tl_namespace', 'tl_title' ),
		),
		'externallinks' => array(
			'src_id' => 'el_from',
			'url' => 'el_to',
			'_indexes' => array( 'el_from', 'el_to' ),
		),
	);
} );
