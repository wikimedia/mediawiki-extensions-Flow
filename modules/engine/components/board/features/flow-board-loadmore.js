...
( function ( $, mw ) {
	...
	function _flowBoardComponentLoadMoreFeatureInfiniteScrollCheck( $container, event ) {
		...
		if ( !event ) {
			if ( $container.data && $container.data[ 0 ] ) {
				/** @type {Event} */
				event = $container;
				/** @type {jQuery} */
		...
		if ( !$container ) {
			return; // @todo debug why this is happening
		}

		// This is probably window, but for TOC it is another scrollable container
		var $scrollContainer = $( event.target === document ? window : event.target ),
			scrollContainerHeight = $scrollContainer.height(),
			scrollContainerScroll = $scrollContainer.scrollTop(),
			scrollContainerThreshold = ( $scrollContainer.offset() || { top: scrollContainerScroll } ).top;
		...
		$container.find( this.$loadMoreNodes ).filter( ':visible' ).each( function () {
			...
			if ( nodeOffset <= scrollContainerThreshold ) {
				// Top of element is above the viewport; don't use it.
				return;
			}

			// Is this element in the viewport?
			if ( nodeOffset - nodeHeight <= scrollContainerThreshold + scrollContainerHeight ) {
