...
class SpecialFlow extends SpecialPage {
	...
	public function execute( $subPage ) {
		...
		$workflowId = $request->getVal( 'workflowId' );
	...
	protected function loadDefinition() {
		...
		$id = $this->getRequest()->getVal( 'flowId' );
		if ( $id !== null ) {
	...
	protected function handleSubmit( Workflow $workflow, Definition $definition, $action, array $blocks ) {
		...
		foreach ( $blocks as $block ) {
			...
			if ( $result !== null ) {
				...
				echo get_class( $block ) . ':' . var_export( $result, true ) . "<br>";
				$success &= $result;
			}
		}
		if ( !$blocksToCommit ) {
		...
		return $success ? $blocksToCommit : array();
	}
	...
	protected function redirect( Workflow $workflow, $action = 'view', $flow = null ) {
		$url = $this->container['url_generator']->generateUrl( $workflow, $view, $flow );
