...
class BoardHistoryIndex extends TopKIndex {
	...
	public function onAfterInsert( $object, array $new ) {
		if ( $new['rev_type'] === 'header' ) {
			...
			$topicListId = $this->findTopicListIdForRootPost( $object, $new['tree_rev_descendant_id'] );
			if ( $topicListId ) {
	...
	public function onAfterUpdate( $object, array $old, array $new ) {
		if ( $new['rev_type'] === 'header' ) {
			...
			$topicListId = $this->findTopicListIdForRootPost( $object, $old['tree_rev_descendant_id'] );
			if ( $topicListId ) {
	 ...
	 */
	protected function findTopicListIdForRootPost( $object, $postId ) {
		if ( !$object->isTopicTitle() ) {
			return false;
		}

		$post = UUID::create( $postId );
