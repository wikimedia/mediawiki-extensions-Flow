...
class Query {
	...
	public function getResults( ContribsPager $pager, $offset, $limit, $descending ) {
		...
		$this->loadMetadataBatch( $results );
	...
	protected function loadMetadataBatch( $results ) {
		...
		$rootPostIds = $treeRepo->findRoots( $postIds );
		// No harm in searching for a workflow that doesn't exist
		$workflows = $storage->getMulti( 'Workflow', array_merge( $rootPostIds, $postIds, $workflowIds ) );
		$rootPosts = $storage->getMulti( 'PostRevision', $rootPostIds );
		...
		$this->rootPostCache = array_merge( $this->rootPostCache, $rootPosts );
	}
