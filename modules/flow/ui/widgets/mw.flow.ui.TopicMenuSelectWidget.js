( function ( $ ) {
	...
	mw.flow.ui.TopicMenuSelectWidget.prototype.onTopicContentChange = function ( topic, content ) {
		var topicWidget = this.getItemFromData( topic.getId() );
	 ...
	 * @fires topic
	 */
	mw.flow.ui.TopicMenuSelectWidget.prototype.onTopicChoose = function ( topicId ) {
		if ( topicId ) {
	...
	mw.flow.ui.TopicMenuSelectWidget.prototype.addTopics = function ( items, index ) {
		...
		for ( i = 0, len = items.length; i < len; i++ ) {
			optionWidget = this.getItemFromData( items[ i ].getId() );
			if ( !optionWidget ) {
