<?php use \LightnCandy\SafeString as SafeString;use \LightnCandy\Runtime as LR;return function ($in, $options = null) {
    ...
    return ''.LR::sec($cx, ((isset($in['blocks']) && is_array($in)) ? $in['blocks'] : null), null, $in, true, function($cx, $in) {return '	'.LR::encq($cx, LR::hbch($cx, 'block', array(array($in),array()), 'encq', $in)).'
