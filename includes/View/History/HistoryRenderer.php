...
class HistoryRenderer {
	...
	public function getTimespans( History $history ) {
		...
		do {
			...
			$previous = end( $timespans )['from']->getTimestamp( TS_UNIX );
			$start = new MWTimestamp( $start );
			$end = new MWTimestamp( min( $end, $previous ) ); // @todo: fix this

			// Build message.
			$text = $wgLang->sprintfDate( 'F Y', $start->getTimestamp( TS_MW ) );
		...
		// keep going until we've reached the oldest timestamp (or failsafe counter runs out)
		} while ( $lastTimestamp < $start->getTimestamp( TS_UNIX ) || $i++ < 9999 );
