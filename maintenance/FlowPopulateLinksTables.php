 ...
 * of each object.
 */
class FlowPopulateLinksTables extends Maintenance {
	public function __construct() {
		parent::__construct();
		$this->mDescription = "foobar";
	}

	public function execute() {
		$recorder = Flow\Container::get( 'reference.recorder' );
		$this->processHeaders( $recorder );
		$this->processPosts( $recorder );
	}

	protected function processHeaders( $recorder ) {
		$storage = Flow\Container::get( 'storage.header' );
	...
	protected function processPosts( $recorder ) {
		$storage = Flow\Container::get( 'storage.post' );
		$count = $this->mBatchSize;
		$id = '';
		$dbr = Flow\Container::get( 'db.factory' )->getDB( DB_SLAVE );
		while ( $count === $this->mBatchSize ) {
