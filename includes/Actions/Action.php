...
class FlowAction extends Action {
	...
	protected function detectWorkflowId( Title $title, WebRequest $request ) {
		if ( $title->getNamespace() === NS_TOPIC ) {
			// @todo for now throwing away everything after the workflow id, but in the future
			// we should redirect to whatever the current topic title is

			// Title::getBaseText wont return what we need because subpages are disabled
			// in the topic namespace
			$pieces = explode( '/', $title->getText() );

			$uuid = UUID::create( $pieces[0] );
			if ( !$uuid ) {
				// @todo better error handling
