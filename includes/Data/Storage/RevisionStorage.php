...
abstract class RevisionStorage extends DbStorage {
	...
	public function isUpdatingExistingRevisionContentAllowed() {
		...
		$columnAllowedArray = array();

		foreach( $requiredColumnNames as $columnName ) {
			$columnAllowedArray[] = in_array( $columnName, $this->allowedUpdateColumns, true );
		}

		$first = $columnAllowedArray[0];
		$all = $first;
		foreach ( $columnAllowedArray as $isAllowed ) {
			if ( $isAllowed !== $first ) {
	 ...
	 */
	public function calcUpdates( $old, $new ) {
		...
		$validatedChangeset = $this->calcUpdates( array(), $unvalidatedChangeset );
