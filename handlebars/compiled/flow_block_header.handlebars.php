...
'.$sp.'		  title="'.LCRun3::ch($cx, 'l10n', array(array('flow-board-expand-description'),array()), 'encq').'"></span>
'.$sp.'</a>
';},'flow_header_categories' => function ($cx, $in, $sp) {return ''.$sp.'<div class="flow-board-header-category-view">
'.$sp.'	<ul class="flow-board-header-category-list">
'.$sp.''.LCRun3::sec($cx, ((isset($cx['sp_vars']['root']['categories']) && is_array($cx['sp_vars']['root'])) ? $cx['sp_vars']['root']['categories'] : null), $in, true, function($cx, $in)use($sp){return '		<li>'.LCRun3::ch($cx, 'html', array(array($in),array()), 'encq').'</li>
'.$sp.'';}).'	</ul>
'.$sp.'</div>
