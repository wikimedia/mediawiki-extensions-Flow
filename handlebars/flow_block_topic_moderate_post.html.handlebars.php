...
';}).'
	...
	<div class="flow-form-actions flow-form-collapsible">
		...
		<button data-flow-interactive-handler="createForm" data-role="cancel" class="flow-ui-button flow-ui-destructive flow-ui-quiet">'.LCRun2::ch('l10n', Array('flow-cancel'), 'enc', $cx).'</button>
	</div>
</form>
	...
	<div class="flow-post'.LCRun2::ifv(((is_array($in) && isset($in['isModerated'])) ? $in['isModerated'] : null), $cx, $in, function($cx, $in) {return ' flow-post-moderated';}).'">
		'.LCRun2::wi(((is_array($in) && isset($in['author'])) ? $in['author'] : null), $cx, $in, function($cx, $in) {return '
			<span class="flow-author"><a href="'.htmlentities(((is_array($in['links']['contribs']) && isset($in['links']['contribs']['url'])) ? $in['links']['contribs']['url'] : null), ENT_QUOTES, 'UTF-8').'" title="'.htmlentities(((is_array($in['links']['contribs']) && isset($in['links']['contribs']['title'])) ? $in['links']['contribs']['title'] : null), ENT_QUOTES, 'UTF-8').'" class="mw-userlink flow-ui-tooltip-target">'.htmlentities(((is_array($in) && isset($in['name'])) ? $in['name'] : null), ENT_QUOTES, 'UTF-8').'</a> <span class="mw-usertoollinks">(<a href="'.htmlentities(((is_array($in['links']['talk']) && isset($in['links']['talk']['url'])) ? $in['links']['talk']['url'] : null), ENT_QUOTES, 'UTF-8').'" class="new flow-ui-tooltip-target" title="'.htmlentities(((is_array($in['links']['talk']) && isset($in['links']['talk']['title'])) ? $in['links']['talk']['title'] : null), ENT_QUOTES, 'UTF-8').'">'.LCRun2::ch('l10n', Array('talkpagelinktext'), 'enc', $cx).'</a>'.LCRun2::ifv(((is_array($in['links']) && isset($in['links']['block'])) ? $in['links']['block'] : null), $cx, $in, function($cx, $in) {return ' | <a class="flow-ui-tooltip-target" href="'.htmlentities(((is_array($in['links']['block']) && isset($in['links']['block']['url'])) ? $in['links']['block']['url'] : null), ENT_QUOTES, 'UTF-8').'" title="'.htmlentities(((is_array($in['links']['block']) && isset($in['links']['block']['title'])) ? $in['links']['block']['title'] : null), ENT_QUOTES, 'UTF-8').'">'.LCRun2::ch('l10n', Array('blocklink'), 'enc', $cx).'</a>';}).')</span></span>
		';}).'
		...
		<div class="flow-menu">
			...
			<ul class="flow-ui-button-container">
				'.LCRun2::ifv(((is_array($in['actions']) && isset($in['actions']['lock'])) ? $in['actions']['lock'] : null), $cx, $in, function($cx, $in) {return '
					<li><a class="flow-ui-button flow-ui-progressive flow-ui-quiet flow-ui-thin" href="'.htmlentities(((is_array($in['actions']['lock']) && isset($in['actions']['lock']['url'])) ? $in['actions']['lock']['url'] : null), ENT_QUOTES, 'UTF-8').'" title="'.htmlentities(((is_array($in['actions']['lock']) && isset($in['actions']['lock']['title'])) ? $in['actions']['lock']['title'] : null), ENT_QUOTES, 'UTF-8').'"><span class="wikicon wikicon-eye-lock"></span> '.LCRun2::ch('l10n', Array('TODO-Lock'), 'enc', $cx).'</a></li>
				';}).'
