	...
	'.LCRun3::ch($cx, 'l10nParse', Array(Array('flow-anon-warning',LCRun3::ch($cx, 'linkWithReturnTo', Array(Array('Special:UserLogin'),Array()), 'raw'),LCRun3::ch($cx, 'linkWithReturnTo', Array(Array('Special:UserLogin/signup'),Array()), 'raw')),Array()), 'encq').'';}).'
		...
		<div class="flow-form-actions flow-form-collapsible">
			<button data-role="submit" data-flow-api-handler="newTopic"
				data-flow-interactive-handler="apiRequest"
				class="flow-ui-button flow-ui-constructive">'.LCRun3::ch($cx, 'l10n', Array(Array('flow-newtopic-save'),Array()), 'encq').'</button>
...
<div class="flow-topic-summary-container">
	...
	'.((LCRun3::ifvar($cx, ((is_array($in) && isset($in['isAlwaysWatched'])) ? $in['isAlwaysWatched'] : null))) ? '
		<span class="flow-ui-quiet flow-ui-thin flow-ui-constructive flow-ui-active">'.htmlentities(((is_array($in) && isset($in['null'])) ? $in['null'] : null), ENT_QUOTES, 'UTF-8').'<span class="wikiglyph wikiglyph-star"></span>'.htmlentities(((is_array($in) && isset($in['null'])) ? $in['null'] : null), ENT_QUOTES, 'UTF-8').'</span>
	' : '
		...
		<div class="flow-menu">
			...
			<ul class="flow-ui-button-container">
				...
				'.((LCRun3::ifvar($cx, ((is_array($in['links']) && isset($in['links']['topic-history'])) ? $in['links']['topic-history'] : null))) ? '
					<li '.((LCRun3::ifvar($cx, ((is_array($in['actions']) && isset($in['actions']['edit'])) ? $in['actions']['edit'] : null))) ? ''.LCRun3::hbch($cx, 'doOnce', Array(Array(((is_array($in) && isset($in['\'splitlinks\''])) ? $in['\'splitlinks\''] : null),((is_array($in) && isset($in['postId'])) ? $in['postId'] : null)),Array()), $in, function($cx, $in) {return 'class="flow-menu-split"';}).'' : '').'>
						<a class="flow-ui-button flow-ui-quiet flow-ui-thin"
				...
				'.((LCRun3::ifvar($cx, ((is_array($in['actions']) && isset($in['actions']['lock'])) ? $in['actions']['lock'] : null))) ? '
					<li '.LCRun3::hbch($cx, 'doOnce', Array(Array(((is_array($in) && isset($in['\'splitactions\''])) ? $in['\'splitactions\''] : null),((is_array($in) && isset($in['postId'])) ? $in['postId'] : null)),Array()), $in, function($cx, $in) {return 'class="flow-menu-split"';}).'>
