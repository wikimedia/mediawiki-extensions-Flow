...
class RevisionActionPermissions {
	...
	public function isAllowed( AbstractRevision $revision, $action ) {
		...
		if ( !isset( $last[$revisionId] ) ) {
			/** @var CollectionCache $cache */
			$cache = Container::get( 'collection.cache' );
			$last[$revisionId] = $cache->getLastRevisionFor( $revision );
		}
		...
		$isLastRevision = $last[$revisionId]->getRevisionId()->equals( $revision->getRevisionId() );
		if ( !$isLastRevision && !$this->isRevisionAllowed( $last[$revisionId], $action ) ) {
			return false;
		}
