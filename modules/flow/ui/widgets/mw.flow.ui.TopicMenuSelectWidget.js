( function ( $ ) {
	...
	mw.flow.ui.TopicMenuSelectWidget.prototype.onMenuScroll = function () {
		...
		if (
			isNearBottom ||
			(
				// See if the widget is attached at all
				$loadMore.parent() &&
				$loadMore.offset().top < this.$element.scrollTop + this.$element.height()
			)
		) {
			this.getMoreTopics();
		}
	...
	mw.flow.ui.TopicMenuSelectWidget.prototype.getMoreTopics = function () {
		...
		return this.api.getTopicList(
			...
			.then( function ( topics ) {
				...
				widget.noMoreTopics = ( topics.length < widget.tocPostLimit ) ;
				// Add to board
