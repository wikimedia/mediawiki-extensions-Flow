...
abstract class FlowPresentationModel extends EchoEventPresentationModel {
	 ...
	 */
	protected function getTopicTitle( $fragment = '' ) {
	...
	protected function getFlowUnwatchDynamicActionLink( Title $title, $isTopic = false ) {
		...
		$messageKeys = array(
			'item' => array(
				...
				// notifications-dynamic-actions-flow-topic-unwatch
				'title' => $this
					->msg( 'notifications-dynamic-actions-flow-' . $type . '-unwatch' )
					->params(
			...
			'confirmation' => array(
				...
				// notifications-dynamic-actions-flow-topic-unwatch-confirmation
				'title' => $this
					->msg( 'notifications-dynamic-actions-flow-' . $type . '-unwatch-confirmation' )
					->params(
						$title->getPrefixedText(),
						$title->getFullURL( $query )
					)
					->parse(),
				// notifications-dynamic-actions-flow-board-unwatch-confirmation-description
				// notifications-dynamic-actions-flow-topic-unwatch-confirmation-description
				'description' => $this
					->msg( 'notifications-dynamic-actions-flow-' . $type . '-unwatch-confirmation-description' )
					->params(
						$title->getPrefixedText(),
						$title->getFullURL( $query )
					)
