...
( function ( $, mw ) {
	 ...
	 */
	FlowComponentEventsMixin.prototype.UI = {
		events: {
	...
	function emitWithReturn( event, args ) {
		...
		mw.flow.debug( 'Found no event handlers to emit.', arguments );
		return false;
	}
	...
	function flowEventsMixinApiRequestInteractiveHandler( event ) {
		...
		if ( flowComponent.UI.events.apiPreHandlers[ handlerName ] ) {
			...
			if ( preHandlerReturn === false || preHandlerReturn._abort === true ) {
				...
				flowComponent.debug( 'apiPreHandler returned false', handlerName, args );
		...
		if ( flowComponent.UI.events.apiHandlers[ handlerName ] ) {
			$deferred
				...
				.fail( function ( code, result ) {
					 ...
					 */
					errorMsg = _apiErrorMessage( code, result );
					flowComponent.emitWithReturn( 'showError', info.$target, errorMsg );
	...
	FlowComponentEventsMixin.eventHandlers.apiHandler = flowInteractiveHandlerCallback;
	...
	function flowEventsMixinInitializeEditors( $container ) {
		mw.loader.using( 'ext.flow.editor', function() {
			...
			$editors.each( function() {
				var $editor = $( this ),
					flowComponent = mw.flow.getPrototypeMethod( 'component', 'getInstanceByElement' )( $editor );
	...
	FlowComponentEventsMixin.eventHandlers.menuToggle = flowEventsMixinMenuToggle;
	...
	mw.flow.extendComponent( 'component', FlowComponentEventsMixin );
}( jQuery, mediaWiki ) );