...
( function ( $, mw ) {
	...
	/**
	 * Adds a hardcoded moderation reason to the request when resolving and re-opening a topic.
	 * @param {Event} event
	 * @param {Object} info
	 * @param {Object} queryMap
	...
	FlowBoardComponentApiEventsMixin.UI.events.apiHandlers.lockTopic = function ( info, data ) {
		...
		return _flowBoardComponentRefreshTopic( info.$target, workflowId ).done( summarize );
	};
	...
	function _activateSummarizeTopic( $target, flowBoard, topicSummary, action ) {
		...
		// @todo: pass 'action' to the template to customize the buttons
		...
		$target = $( flowBoard.constructor.static.TemplateEngine.processTemplateGetFragment(
			...
			jQuery.extend( topicSummary, { action: action } )
		) ).children();
