...
' : '').'
	<form method="POST" action="'.LR::encq($cx, ((isset($in['links']['undo-edit-header']) && is_array($in['links']['undo-edit-header']) && isset($in['links']['undo-edit-header']['url'])) ? $in['links']['undo-edit-header']['url'] : null)).'" class="flow-post" data-module="topicsummary">
		...
		<input type="hidden" name="topicsummary_prev_revision" value="'.LR::encq($cx, ((isset($in['current']) && is_array($in['current']) && isset($in['current']['revisionId'])) ? $in['current']['revisionId'] : null)).'" />

		<div class="flow-editor">
			<textarea name="topicsummary_summary"
			          class="mw-ui-input mw-editfont-'.LR::encq($cx, (isset($cx['sp_vars']['root']['rootBlock']['editFont']) ? $cx['sp_vars']['root']['rootBlock']['editFont'] : null)).'"
			          data-role="content"
		...
		<div class="flow-form-actions flow-form-collapsible">
			<button class="mw-ui-button mw-ui-progressive">'.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-topic-action-summarize-topic'),array()), 'encq', $in)).'</button>
