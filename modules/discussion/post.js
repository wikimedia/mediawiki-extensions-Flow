( function ( $, mw ) {
	...
	mw.flow.action.post.edit = function ( post ) {
		...
		this.post.$container.find( '.flow-edit-post-link' ).on( 'click.mw-flow-discussion', this.edit.bind( this ) );
	};
...
//		deferred.fail( this.conflict.bind( this, deferred ) ); // @todo: not yet implemented
	...
	mw.flow.action.post.reply = function ( post ) {
		...
		// Overload "reply" link.
		this.post.$container.find( '.flow-reply-link' ).on( 'click.mw-flow-discussion', this.reply.bind( this ) );
	};
