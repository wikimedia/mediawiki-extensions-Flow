...
class ApiBackend {
	...
	public function retrieveThreadData( array $conds ) {
		...
		if ( ! isset( $data['query']['threads'] ) ) {
			throw new ApiException( "Null response from API module", $data );
		}

		$firstThread = reset( $data['query']['threads'] );
		if ( ! isset( $firstThread['replies'] ) ) {
			throw new ApiException( "Foreign API does not support reply exporting", $data );
		}
...
class ApiException extends ImportException {
	protected $apiResponse;

	public function __construct( $message, $apiResponse ) {
	...
	public function isNotFoundError() {
		// This error is output by the api when requesting a deleted post we
		// don't have access to
		$expect = 'Exception Caught: DatabaseBase::makeList: empty input for field thread_parent';
		return $this->apiResponse['error']['info'] === $expect;
	}
}
