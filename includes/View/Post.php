...
class Post {
	...
	public function getDiffLink( $block ) {
	...
	public function createModifiedTipsyLink( $block ) {
		if ( $this->post->getPrevRevisionId() ) {
			$link = Html::element(
				'a',
				array( 'class' => 'flow-content-modified-tipsy-link', 'href' => $this->getDiffLink( $block ) ),
	...
	public function createModifiedTipsyHtml( $block ) {
		...
		if ( $this->post->getLastContentEditorName() ) {
			...
			$html .= Html::element( 'a', array( 'href' => $this->getDiffLink( $block ) ), wfMessage( 'flow-show-change' )->text() );
