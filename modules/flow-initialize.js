	...
	$( document ).ready( function () {
		...
		function startEditTopicSummary( topicId, action ) {
			var $topic = $board.find( '#flow-topic-' + topicId ),
				$summaryElement = $topic.find( '.flow-topic-summary' ),
			...
			editTopicSummaryWidget
				...
				.on( 'cancel', function () {
					editTopicSummaryWidget.$element.replaceWith( $summaryElement );
					editTopicSummaryWidget.destroy();
				} );

			$summaryElement.replaceWith( editTopicSummaryWidget.$element );
		}
