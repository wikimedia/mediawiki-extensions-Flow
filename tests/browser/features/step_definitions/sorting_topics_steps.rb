...
When(/^I click Recently active topics link$/) do
  on(FlowPage) do |page|
    page.wait_until do
    page.recently_active_topics_choice_element.visible? == false
    end
...
Then(/^the Flow page should not show Recently active topics link$/) do
  on(FlowPage) do |page|
    page.wait_until do
      page.recently_active_topics_link_element.visible? == false
    end
...
Then(/^the Flow page should not show Newest topics link$/) do
  on(FlowPage) do |page|
    page.wait_until do
    page.newest_topics_link_element.visible? == false
    end
