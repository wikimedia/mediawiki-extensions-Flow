	...
	public function getMulti( array $topicIds ) {
		...
		foreach ( $found as $indexResult ) {
			...
			if ( isset( $posts[$post->getPostId()->getHex()] ) ) {
				throw new InvalidDataException( 'Multiple results for id: ' . $post->getPostId()->getHex(), 'fail-load' );
			}
		...
		if ( $missing ) {
			...
			throw new InvalidDataException( 'Missing Posts: ' . json_encode( $missing ), 'fail-load' );
		}
		// another helper to catch bugs in dev
		$extra = array_diff( array_keys( $posts ), $prettyPostIds );
		if ( $extra ) {
			throw new InvalidDataException( 'Found unrequested posts: ' . json_encode( $extra ), 'fail-load' );
		}
		$extraParents = array_diff( array_keys( $children ), $prettyPostIds );
		if ( $extraParents ) {
			throw new InvalidDataException( 'Found posts with unrequested parents: ' . json_encode( $extraParents ), 'fail-load' );
		}
