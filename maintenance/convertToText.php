...
class ConvertToText extends Maintenance {
	...
	public function execute() {
		...
		assert_options( ASSERT_CALLBACK, function( $file, $line, $code ) {
			die( "Assertion $code failed at $file:$line\n" );
		} );
		...
		while( $continue ) {
			...
			assert( "isset( \$apiResponse['query']['flow'] )" );
			$flowData = $apiResponse['query']['flow'];

			assert( '$flowData["_element"] === "block"' );
			...
			assert( '$topicListBlock !== false' );
			foreach( $topicListBlock as $key => $topic ) {
				if ( ! is_numeric( $key ) ) {
					continue;
				}

				assert( 'isset( $topic["topic-id"] )' );
	...
	public function processPostCollection( $collection, $indentLevel = 0 ) {
		assert( '$collection["_element"] === "post"' );
