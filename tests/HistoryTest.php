...
class HistoryTest extends PostRevisionTestCase {
	...
	static public function recordGroupingProvider() {
		return array(
			array(
				'Records from months ago are grouped by month',
				array(
					'February 2014' => 1,
					'January 2014' => 3,
					'December 2013' => 1,
			...
			array(
				// @todo this currently fails, should it be true?
				'bugged records from the future are a member of last 4 hours',
				array(
					'Last 4 hours' => 1,
				),
				array(
					wfTimestamp( TS_MW, time() + ( 60 * 60 * 24 * 7 ) )
				),
			),
			...
			array(
				// @todo this fails localy
				'3.5 hours ago is a member of last 4 hours',
				array(
					'Last 4 hours' => 1,
				),
				array(
					wfTimestamp( TS_MW, time() - (int)( 60 * 60 * 3.5 ) ),
				),
			),

			array(
				'4 hours ago is last week',
				array(
					'Last week' => 1,
				),
				array(
					wfTimestamp( TS_MW, time() - ( 60 * 60 * 4 ) ),
				),
			),
		);
	}
	 ...
	 */
	public function testRecordGrouping( $message, array $expected, array $timestamps ) {
		// meh
