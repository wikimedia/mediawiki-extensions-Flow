...
class LqtNotifications implements Postprocessor {
	...
	/**
	 * @var int|null Either the id of the lqt thread that is having notifications sent,
	 *  or null if not active.
	 */
	protected $activeThreadId = null;
	...
	public function __construct( NotificationController $controller, DatabaseBase $dbr ) {
	...
	public function locateUsersWithPendingLqtNotifications( EchoEvent $event, $batchSize = 500 ) {
		...
		$it->addConditions( array(
			'ums_thread' => $this->activeThreadId,
		) );

		// flatten result into a stream of rows
		$it = new RecursiveIteratorIterator( $it );

		// add callback to onvert user id to user objects
	...
	public function afterTopicImported( TopicImportState $state, IImportTopic $topic ) {
		...
		$this->activeThreadId = $topic->getLqtThreadId();
