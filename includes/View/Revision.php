...
class PostSummaryRevisionView extends RevisionView {
	 ...
	 * @param User $user
	 */
	public function __construct( TopicSummary $revision, Templating $templating, TopicSummaryBlock $block, User $user ) {
		parent::__construct( $templating, $user );
		$this->revision = $revision;
		$this->block = $block;
	}
	...
	public function isComparableTo( AbstractRevision $revision ) {
		if ( $revision instanceof TopicSummary ) {
			return $this->revision->getSummaryTargetId()->equals( $revision->getSummaryTargetId() );
		} else {
			return false;
		}
	}
	...
	public function getSingleViewHeader() {
		...
		$msgKey = $compareLink ? 'flow-revision-permalink-warning-post' : 'flow-revision-permalink-warning-postsummary-first';
		$message = wfMessage( $msgKey )
			->params( $this->revision->getRevisionId()->getTimestampObj()->getHumanTimestamp() )
			->params(
				$this->block->getWorkflow()->getArticleTitle(),
