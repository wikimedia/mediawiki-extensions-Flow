...
class View extends ContextSource {
	...
	protected function buildApiResponse( WorkflowLoader $loader, array $blocks, $action, array $parameters ) {
		...
		$userPrefSort = $user->getOption( 'flow-topiclist-sortby' );
		...
		// may ignore.
		if ( $userPrefSort === 'newest' ) {
			// Make sure we found topiclist block
			// and that it actually has roots in it
			$existingRoots = $topiclistBlockIndex > -1 &&
				$apiResponse['blocks'][$topiclistBlockIndex] !== undefined &&
				is_array( $apiResponse['blocks'][$topiclistBlockIndex]['roots'] ) ?
					$apiResponse['blocks'][$topiclistBlockIndex]['roots'] :
					array();
