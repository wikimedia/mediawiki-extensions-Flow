( function ( $ ) {
	...
	mw.flow.Initializer.prototype.setupHistoryTab = function () {
		if ( $( '#ca-history' ).length === 0 ) {
			var portlet, skin, tabText, nextnode, portletList;
			...
			// Choose the right place to put the History tab
			if ( portlet === 'p-views' ) {
				// For Vector etc. the tab is the first item in the appropriate porlet
				portletList = $( '#' + portlet ).find( 'ul' ).first();
				nextnode = portletList.find( 'li' ).first();
			} else if ( portlet === 'p-cactions' ) {
				// For Monobook etc. the tab is the first item after the discussion tab
				nextnode = $( '#ca-talk' ).next();
			}
