	...
	( function () {
		...
		FlowBoardComponent.UI.events.apiHandlers.closeOpenTopic = function ( status, data, jqxhr ) {
			...
			if ( status === 'done' ) {
				...
				// @todo Rebuild reopen ( close ) link from api response
				// and replace the close ( reopen) link in action menu
				// or just rebuild the entire action menu based on API result
			} else {
				// @todo fail
				alert('fail');
			}
