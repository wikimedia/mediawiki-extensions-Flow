	...
	<div class="flow-board-header-edit-view">
		...
		<form method="POST" action="'.htmlentities(((is_array($in['revision']['actions']['edit']) && isset($in['revision']['actions']['edit']['url'])) ? $in['revision']['actions']['edit']['url'] : null), ENT_QUOTES, 'UTF-8').'" flow-api-action="edit-header">
			...
			<div class="flow-form-actions flow-form-collapsible">
				...
				<a class="flow-ui-button flow-ui-destructive"
				   href="'.htmlentities(((is_array($in['revision']['links']['board']) && isset($in['revision']['links']['board']['url'])) ? $in['revision']['links']['board']['url'] : null), ENT_QUOTES, 'UTF-8').'"
				   data-flow-interactive-handler="cancelForm"
				   data-role="cancel"
				   flow-ui-quiet flow-click-interactive">'.LCRun3::ch($cx, 'l10n', Array('flow-cancel'), 'encq').'</a>
