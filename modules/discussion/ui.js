(function ( $, mw ) {
	var FlowUI = {
		...
		events: {
			...
			replyClick: function ( event ) {
				...
				if ( $this.is( '.flow-topic-comments-link' ) ) {
					...
					$formContainer = $this.closest( '.flow-topic-container' ).find( '.flow-topic-reply-container' );

					// Expand the topic if it is currently collapsed
					FlowUI.topicExpand( $formContainer.closest( '.flow-topic-children-container' ), 0 );

					// Focus onto the textarea
					$textarea = $formContainer.find( 'textarea' ).focus();
				} else {
			...
			topicCollapserClick: function (event) {
				...
				if ( collapseClass === undefined ) {
					...
					FlowUI.topicCollapse( $container.addClass( collapseClass ).find( '.flow-topic-children-container' ) );
				}
			},
			...
			titlebarClick: function ( event ) {
				...
				if ( $topicContainer.is( '.flow-topic-closed' ) ) {
					...
					FlowUI.topicExpand( $topicContainerChildren );
				} else {
					// Otherwise, collapse
					FlowUI.topicCollapse( $topicContainerChildren );
				}
			},
		...
		setupGeneric: function () {
			...
			$( '<a/>', {
				'href':  '#',
				'class': 'flow-cancel-link mw-ui-button mw-ui-quiet',
