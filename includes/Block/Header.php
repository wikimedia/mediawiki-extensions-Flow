...
class HeaderBlock extends AbstractBlock {
	...
	protected function validate() {
		...
		if ( $this->header ) {
			...
			$status = AbuseFilterUtils::validate( $this->header, $oldHeader, $this->workflow->getArticleTitle() );
			if ( !$status->isOK() ) {
			...
			if ( empty( $this->submitted['prev_revision'] ) ) {
				...
				// run through AbuseFilter
				$status = AbuseFilterUtils::validate( $this->header, null, $this->workflow->getArticleTitle() );
				if ( !$status->isOK() ) {
					foreach ( $status->getErrorsArray() as $message ) {
						$this->addError( 'abusefilter', wfMessage( array_shift( $message ), $message ) );
					}
