...
'.$sp.'' : '').'</div>
';},
'flow_moderate_topic' => function ($cx, $in, $sp) {return ''.$sp.'<form method="POST" action="'.LR::encq($cx, ((isset($in['moderationAction']) && is_array($in)) ? $in['moderationAction'] : null)).'">
...
'.$sp.'		   data-flow-interactive-handler="cancelForm">'.LR::encq($cx, ((isset($in['l10n']) && is_array($in)) ? $in['l10n'] : null)).'</a>
'.$sp.'	</div>
'.$sp.'</form>
';},
