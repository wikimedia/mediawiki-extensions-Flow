...
( function ( $, mw ) {
	...
	function flowBoardComponentLoadMoreFeatureElementLoadTopic( $topic ) {
		...
		if ( this.orderedTopicIds.indexOf( currentTopicId ) === -1 ) {
	...
	function _flowBoardComponentLoadMoreFeatureRenderTopics( flowBoard, topicsData, forceShowLoadMore, $insertAt, scrollTarget, scrollContainer, scrollTemplate ) {
		...
		var i,
			$prevExist, $nextExist, foundState, $newTopics = $(),
		...
		for ( i = topicsData.roots.length; i--; ) {
			if ( flowBoard.renderedTopics[ topicsData.roots[ i ] ] ) {
				// This topic was already rendered on the page; don't re-render it
			...
			for ( i = 0; i < flowBoard.orderedTopicIds.length; i++ ) {
				if ( flowBoard.renderedTopics[ flowBoard.orderedTopicIds[i] ] ) {
					if ( foundState ) {
						...
						if ( foundState === 'foundImmediatelyAfter' ) {
							foundState = 'foundImmediatelyBefore';
						}
				...
				if ( flowBoard.orderedTopicIds[ i ] === findTopicId ) {
					...
					foundState = 'foundImmediatelyAfter';

					if ( $prevExist ) {
						if ( flowBoard.renderedTopics[ flowBoard.orderedTopicIds[i+1] ] ) {
							foundState = 'foundImmediatelyAfterAndBefore';
						}

						break; // We have somewhere to insert this
					}
				} else if ( foundState === 'foundImmediatelyAfter' ) {
					foundState = 'foundBetween';
				}
			}
		}
		...
		function _createRevPagination() {
			...
			$newTopics = $( flowBoard.constructor.static.TemplateEngine.processTemplateGetFragment(
					...
					.insertBefore( $newTopics.first() )
					.add( $newTopics );
		}

		/** @private
		 */
		function _createFwdPagination() {
			if ( forceShowLoadMore || topicsData.links.pagination.fwd ) {
				...
				$newTopics = $newTopics.add(
					$( flowBoard.constructor.static.TemplateEngine.processTemplateGetFragment(
						...
						.insertAfter( $newTopics.last() )
				);
			}
		}

		// Figure out where to drop in load more buttons
		switch ( foundState ) {
			case 'foundImmediatelyAfterAndBefore': // found directly between existing topics; no pagination
				break;
			case 'foundBetween': // somewhere between topics, or some time after the last known topic; both pagination!
				_createRevPagination();
				_createFwdPagination();
				break;
			case 'foundImmediatelyAfter': // found directly before an existing topic; create PREV pagination
				_createRevPagination();
				break;
			case 'foundImmediatelyBefore': // found directly after an existing topic; create NEXT pagination
