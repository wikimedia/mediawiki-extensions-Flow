...
class TopicBlock extends AbstractBlock {
	...
	protected function validateEditTopicSummary() {
		...
		if ( !$summary ) {
			if ( !$this->permissions->isAllowed( null, 'edit-topic-summary' ) ) {
				$this->addError( 'permissions', wfMessage( 'flow-error-not-allowed' ) );
				return;
			}
			$this->topicSummary = PostSummary::create( $topicTitle, $this->user, $this->submitted['summary'], 'create-topic-summary' );
		// Edit topic summary
		} else {
			$summary = reset( $summary );
			if ( !$this->permissions->isAllowed( $summary, 'edit-topic-summary' ) ) {
				$this->addError( 'permissions', wfMessage( 'flow-error-not-allowed' ) );
				return;
			}
