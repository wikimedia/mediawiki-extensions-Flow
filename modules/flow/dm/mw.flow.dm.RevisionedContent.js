( function ( $ ) {
	 ...
	 *
	 * @event watchChange
	 * @param {boolean} Revision is watched by the current user
	 */
	 ...
	 * @param {boolean} Revision can be watched by the current user
	 */
	 ...
	 * @param {boolean} Revision is original content, and was never edited
	 */
	...
	mw.flow.dm.RevisionedContent.prototype.populate = function ( data ) {
		...
		this.toggleOriginalContent( !!data.isOriginalContent );

		this.setLastEditId( data.lastEditId );
		this.setLastEditUser( data.lastEditUser );

		// TODO: Check if we actually really need to store and work
		// with both of these objects. Right now we are storing them
		// from the API response, but there may not be a need in the
		// front end.
