			...
			foreach ( $revisions as $revision ) {
				if ( $this->isAllowed( $revision ) ) {
					if ( $prevId !== null ) {
						...
						$property = new \ReflectionProperty( get_class( $revision ), 'prevRevision' );
						$property->setAccessible( true );
						$property->setValue( $revision, $prevId );

						$prevId = null;
					}
					$this->formatRevision( $revision );
				} elseif ( $prevId === null ) {
					// if revision can't be dumped, store it's parent id so we
