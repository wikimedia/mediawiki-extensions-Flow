( function ( $, mw ) {
	...
	mw.flow.editors.contenteditable.prototype.getRawContent = function () {
		...
		$content.find( 'br' ).before( "&lt;br&gt;" );

		// trim leading & trailing whitespace (including <br>)
		return $content.text().replace( /^(\s|<br>)+|(\s|<br>)+$/g, '' );
	};
	...
	mw.flow.editors.contenteditable.prototype.resetCursor = function () {
		...
		/*
		 * Selection range is not collapsed (different start & end), we're good.
		 * We don't want to collapse it or we'd break changes to selection.
		 */
		if ( !range.collapsed ) {
			return;
		}

		// we're already inside p inside contenteditable, so we're good
		if ( $( $cursorNode.closest( 'p' ), this.$target ).length > 0 ) {
			return;
		}
