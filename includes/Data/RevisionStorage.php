...
abstract class RevisionStorage extends DbStorage {
	...
	/**
	 * The rev_type_id search key used by client code
	 * @return string
	 */
	abstract protected function getRevTypeIdSearchKey();
	...
	public function findMulti( array $queries, array $options = array() ) {
		// Convert lookup key header_workflow_id/tree_rev_descendant_id
		// to their db equivalent columns: rev_type/rev_type_id
		$searchKey = $this->getRevTypeIdSearchKey();
		$revType = $this->getRevType();
		foreach ( $queries as &$query ) {
			if ( isset( $query[$searchKey] ) ) {
				$query['rev_type'] = $revType;
				$query['rev_type_id'] = $query[$searchKey];
	...
	protected function findMostRecent( array $queries ) {
		...
		$res = $dbr->select(
			...
			__METHOD__,
			array( 'GROUP BY' => 'rev_type_id, rev_type' )
		);
		if ( !$res ) {
...
class PostRevisionStorage extends RevisionStorage {
	...
	protected function getRevTypeIdSearchKey() {
