( function ( $, mw ) {
	...
	mw.flow.discussion.topic = function ( topicId ) {
		...
		this.actions = {
			...
			// init restore interaction
			restore: new mw.flow.action.topic.restore( this )
		};
	};
	...
	mw.flow.action.topic.summarizebase.prototype.initialize = function() {
		...
		this.$container
			...
			.find( '.flow-' + action + '-topic-link-inline' )
	...
	mw.flow.action.topic.close.prototype.submitFunction = function ( data, content ) {
		var deferred = mw.flow.api.closeTopic( {
	...
	/**
	 * Initialises topic restoring interaction object.
	 *
	 * @param {object} topic
	 */
	mw.flow.action.topic.restore = function( topic ) {
		this.object = topic;
		this.action = 'restore';
		this.$container =  $( '.flow-element-container :first', $( '#flow-topic-' + this.object.topicId ) );
		this.initialize();
	...
	// Extend restore action from "shared functionality" mw.flow.action.topic.summarizebase class
	mw.flow.action.topic.restore.prototype = new mw.flow.action.topic.summarizebase();
	mw.flow.action.topic.restore.prototype.constructor = mw.flow.action.topic.restore;
	 ...
	 * @return {string}
	 */
	mw.flow.action.topic.restore.prototype.getTerms = function() {
	 ...
	 * @return {jQuery.Deferred}
	 */
	mw.flow.action.topic.restore.prototype.submitFunction = function ( data, content ) {
		var deferred = mw.flow.api.closeTopic( {
				workflow: this.object.workflowId,
				page: this.object.pageName
			},
	 ...
	 * @param {object} output
	 */
	mw.flow.action.topic.restore.prototype.render = function ( output ) {
		var $newContainer = $( output.topic.rendered );
		$( '#flow-topic-' + this.object.topicId ).replaceWith( $newContainer );
		$newContainer.trigger( 'flow_init' );
	};
