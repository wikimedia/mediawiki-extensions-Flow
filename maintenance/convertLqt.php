...
class ConvertLqt extends Maintenance {
	...
	protected function processTopmostThread( $thread ) {
		...
		if ( $talkpageWorkflow->isNew() ) {
			$storage->put( $talkPageWorkflow );
			$occupationController->ensureFlowRevision( $thread->article(), $talkpageWorkflow );
		}
		...
		if ( $thread->hasSummary() ) {
			$summaryPage = $thread->summary();
			$summaryRevision = $summaryPage->getRevision();
			$summaryText = $summaryRevision->getContent()->getText();
			$summaryUserId = $summaryRevision->getUser();
			$summaryUserName = $summaryRevision->getUserText();

			if ( $summaryUserId ) {
				$summaryUser = User::newFromId( $summaryUserId );
			} else {
				$summaryUser = User::newFromName( $summaryUserName, false );
			}
		...
		// Create reply for the content of the topmost post
