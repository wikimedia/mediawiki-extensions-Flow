...
<a href="#topics/topics"  data-flow-interactive-handler="collapserGroupToggle" class="flow-board-collapser-full flow-board-navigator-right flow-board-navigator-cap"><span class="wikiglyph wikiglyph-stripe-expanded flow-ui-tooltip-target" title="'.LCRun3::ch($cx, 'l10n', Array(Array('flow-toggle-topics'),Array()), 'encq').'"></span></a>
		...
		<a href="javascript:void(0);"
		   ...
		   data-flow-menu-target="< .flow-board-navigation .flow-menu">'.LCRun3::hbch($cx, 'ifCond', Array(Array(((is_array($in) && isset($in['sortby'])) ? $in['sortby'] : null),'===','updated'),Array()), $in, function($cx, $in) {return '
	...
	'.LCRun3::hbch($cx, 'eachPost', Array(Array($cx['scopes'][0],$in),Array()), $in, function($cx, $in) {return '
		<div class="flow-topic flow-element-collapsible
            ...
            '.((LCRun3::ifvar($cx, ((is_array($in) && isset($in['isModerated'])) ? $in['isModerated'] : null))) ? 'flow-topic-moderated
              '.LCRun3::hbch($cx, 'ifCond', Array(Array(((is_array($in) && isset($in['moderateState'])) ? $in['moderateState'] : null),'!==','close'),Array()), $in, function($cx, $in) {return 'flow-element-collapsed';}).'
            ' : '').'"
...
' : '').'
				...
				>'.LCRun3::ch($cx, 'l10n', Array(Array('flow-reply-topic-title-placeholder',((is_array($in['properties']) && isset($in['properties']['topic-of-post'])) ? $in['properties']['topic-of-post'] : null)),Array()), 'encq').'</a>
			';}).'
		' : '').'
	' : '').'
</div>
