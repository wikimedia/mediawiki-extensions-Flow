...
class TreeRepositoryTest extends \MediaWikiTestCase {
	...
	public function testFailingInsert() {
		$treeRepository = new \Flow\Repository\TreeRepository( $this->mockDbFactory( false ), $this->cache );
		$this->assertNull( $treeRepository->insert( $this->descendant, $this->ancestor ) );
		$this->assertSame( $method->invoke( $treeRepository, 'rootpath', $this->descendant ), false );
		$this->assertSame( $method->invoke( $treeRepository, 'parent', $this->descendant ), false );
	}
